...
>(
    ...
    let root_shared = ds
        .nodes
        .get(&root_addr)
        .map_or(false, |meta| meta.ref_cnt > 1);
...
fn render_ascii_graph<V: Debug, W: Write>(
    ...
    let mut root_line = format!(
        "PathMap[{map_idx}]{}",
        render_value_ptr_part(root_node.inline_value, dc)
    );
...
fn render_path_label(path: &[u8], dc: &DrawConfig) -> String {
    let mode = if dc.ascii_path {
        PathRenderMode::TryAscii
    } else {
        PathRenderMode::ByteList
    };
...
fn viz_map_physical<V: TrieValue + Debug + Hash, A: Allocator>(
    ...
    ds.cmds
        .push(DrawCmd::Map(ds.root, root_node.shared_node_id()));
...
mod test {
    ...
    fn fizzbuzz() {
        ...
        m3_zipper.graft_map(super::utils::ints::gen_int_range::<(), 4, u32>(
            3,
            n as u32,
            3,
            (),
        ...
        m5_zipper.graft_map(super::utils::ints::gen_int_range::<(), 4, u32>(
            5,
            n as u32,
            5,
            (),
        ...
        r_zipper.graft_map(super::utils::ints::gen_int_range::<(), 4, u32>(
            1,
            n as u32,
            1,
            (),
