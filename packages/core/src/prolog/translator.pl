...
translate_expr([H0|T0], Goals, Out) :-
        ...
        ; HV == hyperpose, T = [L]
          -> ( nonvar(L), is_list(L)
               -> build_hyperpose_branches(L, Branches),
                  append(GsH, [member((Goal,Res), Branches), call(Goal), Out = Res], Goals)
        ...
        ; HV == '|->', T = [Args, Body] -> next_lambda_name(F),
                                           ...
                                           exclude_argvars(ArgVars, AllVars, FreeVars),
        ...
        ; translate_args(T, GsT, AVs),
          ...
          ( is_list(AVs), 
            ...
            -> findall(TypeChain, catch(match('&self', [':', Fun, TypeChain], TypeChain, TypeChain), _, fail), TypeChains),
               ( TypeChains \= []
                 -> maplist_typed_functioncall_branch(TypeChains, Fun, T, GsH, IsPartial, Bound, Out, Branches),
                    disj_list(Branches, Disj),
                    Goals = [Disj]
...
hyperpose_runtime(Exprs, Out) :- is_list(Exprs),
                                 member(Expr, Exprs), eval(Expr, Out).
...
maplist_typed_functioncall_branch([TypeChain|TChains], Fun, T, GsH, IsPartial, Bound, Out, [BranchGoal|Branches]) :-
    typed_functioncall_branch(Fun, TypeChain, T, GsH, IsPartial, Bound, Out, BranchGoal),
    maplist_typed_functioncall_branch(TChains, Fun, T, GsH, IsPartial, Bound, Out, Branches).

maplist_bodycall(_, _, [], []).
maplist_bodycall(BodyCallConj, BodyCall, [XVar|L], [Y|Out]) :-
    BodyCallConj,
    ( number(BodyCall) -> Y is BodyCall ; Y = BodyCall ),
    maplist_bodycall(BodyCallConj, BodyCall, L, Out).


foldl_bodycall(_, _, [], Acc, Acc).
foldl_bodycall(BodyConj, BG, [XVar|L], AccVar, Out) :-
    BodyConj,
    ( number(BG) -> NewAcc is BG ; NewAcc = BG ),
    foldl_bodycall(BodyConj, BG, L, NewAcc, Out).

include_condcall(_, _, [], []).
include_condcall(CondConj, CondGoal, [XVar|L], Out) :-
    ( (CondConj, CondGoal == true) ->
        Out = [XVar|RestOut]
    ;
        Out = RestOut
    ),
    include_condcall(CondConj, CondGoal, L, RestOut).

exclude_argvars(_, [], []).
exclude_argvars(ArgVars, [V|AllVars], FreeVars) :-
    ( member_eq(V, ArgVars) ->
        exclude_argvars(ArgVars, AllVars, FreeVars)
    ;
        FreeVars = [V|RestFreeVars],
        exclude_argvars(ArgVars, AllVars, RestFreeVars)
    ).
