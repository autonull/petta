                                       ...
                                       ( ho_specialization(HV, SpecName)
                                         ; ( %4.1. Otherwise register the specialization:
                                             ...
                                             ( %4.2. Re-use the type definition of the parent function for the specialization:
                                               ...
                                               maplist_specializer_clause(SpecName, MetaList, ClauseInfos),
...
specializable_vars(BodyExpr, Value, Arg, HoVars) :- term_variables(Arg, Vars),
                                                    ...
                                                    traverse_list(specializable_vars_predicate, ArgCopy, Value),
...
maplist_specializer_clause(_, [], []).
maplist_specializer_clause(SpecName, [fun_meta(ArgsNorm, BodyExpr)|MetaRest], [clause_info(Input, Clause)|InfoRest]) :-
    Input = [=, [SpecName|ArgsNorm], BodyExpr],
    translate_clause(Input, Clause, false),
    maplist_specializer_clause(SpecName, MetaRest, InfoRest).


specializable_vars_predicate(A, V) :- nonvar(V) -> V = A ; true.
