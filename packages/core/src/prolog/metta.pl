...
'#-'(A, B, R) :- R is A - B.
'#*'(A, B, R) :- R is A * B.
'#div'(A, B, R) :- R is A div B.
'#//'(A, B, R) :- R is A // B.
'#mod'(A, B, R) :- R is A mod B.
'#min'(A, B, R) :- R is min(A,B).
'#max'(A, B, R) :- R is max(A,B).
'#<'(A, B, true)  :- A < B, !.
'#<'(_, _, false).
...
%%% JS bindings: %%%
% This delegates to a JS predicate implemented in tau-prolog environment
'js-call'(SpecList, Result) :-
    ( is_list(SpecList) ->
        SpecList = [Spec|Args],
        ( string(Spec) -> atom_string(A, Spec) ; A = Spec ),
        must_be(atom, A),
        ( atomic_list_concat([Obj, Method], '.', A) ->
            js_call_predicate([Obj, Method | Args], Result)
        ;
            js_call_predicate([global, A | Args], Result)
        )
    ;
        js_call_predicate([global, SpecList], Result)
    ).
...
importer_helper(Space, File) :- atom_string(File, SFile),
                                working_dir(Base),
                                ( file_name_extension(ModPath, 'ts', SFile)
                                  -> absolute_file_name(SFile, Path, [relative_to(Base)]),
                                     js_call_predicate(['__import__', Path], _)
...
:- maplist(register_fun, [superpose, empty, let, 'let*', '+','-','*','/', '%', min, max, 'change-state!', 'get-state', 'bind!',
                          ...
                          decons, 'decons-atom', 'js-call', 'get-type', 'get-metatype', '=alpha', concat, sread, cons, reverse,
