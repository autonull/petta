...
impl SwiplBackend {
    ...
    where
        F: FnOnce(&mut std::process::ChildStdin, &mut BufReader<std::process::ChildStdout>) -> Result<T, Error>,
    {
...
impl super::backend::BackendImpl for MorkBackend {
    ...
    fn load_metta_file(&mut self, path: &Path, _config: &EngineConfig) -> Result<Vec<MettaResult>, Error> {
    ...
    fn process_metta_string(&mut self, code: &str, _config: &EngineConfig) -> Result<Vec<MettaResult>, Error> {
