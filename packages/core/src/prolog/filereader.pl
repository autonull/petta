...
load_metta_file(Filename, Results, Space) :-
    % js_read_file_to_string is a JS predicate
    js_read_file_to_string(Filename, S),
...
process_metta_string(S, Results, Space) :- atom_codes(S, Cs),
                                           strip(Cs, 0, Codes),
...
process_form(Space, parsed(expression, _, Term), []) :- 'add-atom'(Space, Term, true),
                                                        ( silent(true) -> true ; swrite(Term,STerm),
                                                                                 write('--> metta sexpr -->'), nl, write(STerm), nl,
                                                                                 write('^^^^^^^^^^^^^^^^^^^'), nl ).
process_form(_, parsed(runnable, FormStr, Term), Result) :- translate_expr([collapse, Term], Goals, Result),
                                                            ( silent(true) -> true ; write('--> metta runnable -->'), nl, write(FormStr), nl,
...
process_form(Space, parsed(function, FormStr, Term), []) :- add_sexp(Space, Term),
                                                            ...
                                                            ( silent(true) -> true ; write('--> metta function -->'), nl, write(FormStr), nl,
                                                                                     clause(Head, Body, Ref),
                                                                                     ( Body == true -> Show = Head; Show = (Head :- Body) ),
                                                                                     portray_clause(current_output, Show),
                                                                                     write('^^^^^^^^^^^^^^^^^^^^^^'), nl ).
process_form(_, In, _) :- Msg = 'Failed to process', throw(error(syntax_error(Msg), none)).
...
top_forms([Term|Fs], LC0) --> newlines(LC0, LC1),
                              ...
                              ( "(" -> [] ; string_without("\n", Rest), { Msg = 'expected (', throw(error(syntax_error(Msg), none)) } ),
                              ( grab_until_balanced(1, [0'(], Cs, LC1, LC2, 0)
                                -> { true } ; string_without("\n", Rest), { Msg = 'missing )', throw(error(syntax_error(Msg), none)) } ),
                              { atom_codes(FormStr, Cs), Term =.. [Tag, FormStr] },
                              top_forms(Fs, LC2).
