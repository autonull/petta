...
use super::dense_byte::{ByteNode, CellCoFree, CoFree, DenseByteNode, OrdinaryCoFree};
use super::node::*;
...
fn list_node_iterative_drop<V: Clone + Send + Sync, A: Allocator>(node: &mut LineListNode<V, A>) {
    ...
    loop {
        ...
        match next_node.make_mut().into_list() {
            Some(list_node) => match list_node_take_child_to_drop(list_node) {
                Some(child_node) => {
...
#[inline]
fn list_node_take_child_to_drop<V: Clone + Send + Sync, A: Allocator>(
    node: &mut LineListNode<V, A>,
...
impl<V: Clone + Send + Sync, A: Allocator> LineListNode<V, A> {
    ...
    fn get_payload_exact_key_mut<const IS_CHILD: bool>(
        &mut self,
        key: &[u8],
    ) -> Option<&mut ValOrChildUnion<V, A>> {
...
impl<V: Clone + Send + Sync, A: Allocator> LineListNode<V, A> {
        ...
        V: Clone,
    {
        ...
        unsafe {
            child_node.set_payload_0(&node_key_0[idx..], self.is_child_ptr::<0>(), self_payload);
        }

        //Convert slot_0 to a child ptr
        self.val_or_child0 = ValOrChildUnion {
            child: ManuallyDrop::new(TrieNodeODRc::new_in(child_node, self.alloc.clone())),
        };
        ...
        V: Clone,
    {
        ...
        let mut child_node = Self::new_in(self.alloc.clone());
        unsafe {
            child_node.set_payload_0(&node_key_1[idx..], self.is_child_ptr::<1>(), self_payload);
        }

        //Convert slot_0 from to a child ptr
        self.val_or_child1 = ValOrChildUnion {
            child: ManuallyDrop::new(TrieNodeODRc::new_in(child_node, self.alloc.clone())),
        };
    ...
    fn clone_with_updated_payloads(
        &self,
        payload_0: Option<ValOrChildUnion<V, A>>,
        payload_1: Option<ValOrChildUnion<V, A>>,
    ) -> Option<Self> {
        match (payload_0, payload_1) {
            (Some(slot0_payload), Some(slot1_payload)) => {
                ...
                unsafe {
                    new_node.set_payload_0(key0, self.is_child_ptr::<0>(), slot0_payload);
                }
                unsafe {
                    new_node.set_payload_1(key1, self.is_child_ptr::<1>(), slot1_payload);
                }
                debug_assert!(validate_node(&new_node));
                Some(new_node)
            }
            ...
            (None, Some(slot1_payload)) => {
                ...
                unsafe {
                    new_node.set_payload_0(key1, self.is_child_ptr::<1>(), slot1_payload);
                }
                debug_assert!(validate_node(&new_node));
                Some(new_node)
            }
    ...
    #[inline]
    unsafe fn set_payload_0(
        &mut self,
        key: &[u8],
        is_child_ptr: bool,
        payload: ValOrChildUnion<V, A>,
    ) {
        debug_assert!(key.len() <= KEY_BYTES_CNT);
        unsafe {
            core::ptr::copy_nonoverlapping(
                key.as_ptr(),
                self.key_bytes.as_mut_ptr().cast(),
                key.len(),
            );
        }
    ...
    #[inline]
    pub(crate) unsafe fn set_payload_owned<const SLOT: usize>(
        &mut self,
        key: &[u8],
        ...
        V: Clone,
    {
        match SLOT {
            0 => match payload {
                ...
                ValOrChild::Val(val) => unsafe {
                    self.set_payload_0(
                        key,
                        false,
                        ValOrChildUnion {
                            val: ManuallyDrop::new(LocalOrHeap::new(val)),
                        },
                    )
                },
            },
            1 => match payload {
                ValOrChild::Child(child) => unsafe {
                    self.set_payload_1_no_overflow(
                        key,
                        true,
                        ValOrChildUnion {
                            child: ManuallyDrop::new(child),
    ...
    unsafe fn set_payload_0_no_overflow(
        &mut self,
        key: &[u8],
        is_child_ptr: bool,
        payload: ValOrChildUnion<V, A>,
        ...
        V: Clone,
    {
        if key.len() <= KEY_BYTES_CNT {
            ...
            unsafe {
                self.set_payload_0(key, is_child_ptr, payload);
            }
            false
        } else {
            ...
            debug_assert!(!child_node_key.is_empty());
    ...
    /// Creates continuation nodes rather than overflowing the key; returns `true` if a continuation node was
    /// created, or false if everything fit within self
    unsafe fn set_payload_1_no_overflow(
        &mut self,
        key: &[u8],
        is_child_ptr: bool,
        payload: ValOrChildUnion<V, A>,
        ...
        V: Clone,
    {
        ...
        if self.is_available_1() {
            ...
            if key.len() <= remaining_key_bytes {
                //The entire key fits within the node
                unsafe {
                    self.set_payload_1(key, is_child_ptr, payload);
                }
                false
            } else {
                ...
                let mut child_node = Self::new_in(self.alloc.clone());
                unsafe {
                    child_node.set_payload_0_no_overflow(
                        &key[remaining_key_bytes..],
                        is_child_ptr,
                        payload,
                    );
                }
                unsafe {
                    self.set_child_1(
                        &key[..remaining_key_bytes],
                        TrieNodeODRc::new_in(child_node, self.alloc.clone()),
                    );
                }
                true
            }
    ...
    unsafe fn set_payload_0_shift_existing(
        &mut self,
        key: &[u8],
        is_child_ptr: bool,
        payload: ValOrChildUnion<V, A>,
        ...
        V: Clone,
    {
        ...
        if self.is_available_1() {
            ...
            let (new_key, new_is_child_ptr, new_payload, created_sub_branch) =
                if key.len() <= remaining_key_bytes {
                    //The entire key fits within the node
                    ...
                    (
                        ...
                        true,
                        ValOrChildUnion {
                            child: ManuallyDrop::new(TrieNodeODRc::new_in(
                                child_node,
                                self.alloc.clone(),
                            )),
                        },
            ...
            self.header = Self::header0(new_is_child_ptr, new_key_len)
                | Self::header1(old_is_child_ptr, old_key_len);
    ...
    fn swap_payload<const SLOT: usize>(
        &mut self,
        new_payload: ValOrChild<V, A>,
    ) -> ValOrChild<V, A> {
    ...
    pub(crate) fn clone_payload<const SLOT: usize>(&self) -> Option<ValOrChild<V, A>>
    where
        V: Clone,
    {
        if !self.is_used::<SLOT>() {
        ...
        let set_payload_recursive =
            |mut child: TaggedNodeRefMut<'_, V, A>, node_key, payload: ValOrChildUnion<V, A>| {
                if IS_CHILD {
                    ...
                    child
                        .node_set_branch(node_key, onward_link)
                        .map(|_| (None, true))
                } else {
                    let val = unsafe { payload.into_val() };
                    child.node_set_val(node_key, val).map(|(ret_val, _)| {
        ...
        if overlap > 0 && !legal_overlap {
            match merge_guts::<V, A, 0, 1>(overlap, key0, self, key1, self) {
                ...
                AlgebraicResult::Identity(mask) => {
                    ...
                    let mut new_node = Self::new_in(self.alloc.clone());
                    unsafe {
                        new_node.set_payload_owned::<0>(key0, self.clone_payload::<0>().unwrap())
                    };
                    *self = new_node;
                }
    ...
    pub(crate) fn convert_to_dense<Cf: CoFree<V = V, A = A>>(
        &mut self,
        capacity: usize,
    ) -> TrieNodeODRc<V, A>
    where
        ByteNode<Cf, A>: TrieNodeDowncast<V, A>,
    {
        let mut replacement_node =
            ByteNode::<Cf, A>::with_capacity_in(capacity, self.alloc.clone());
        ...
        if self.is_used::<0>() {
            ...
            if key_0.len() > 1 {
                let mut child_node = Self::new_in(self.alloc.clone());
                unsafe {
                    child_node.set_payload_0(&key_0[1..], self.is_child_ptr::<0>(), slot_0_payload);
                }
                replacement_node.set_child(
                    key_0[0],
                    TrieNodeODRc::new_in(child_node, self.alloc.clone()),
                );
            } else {
        ...
        if self.is_used::<1>() {
            ...
            if key_1.len() > 1 {
                let mut child_node = Self::new_in(self.alloc.clone());
                unsafe {
                    child_node.set_payload_0(&key_1[1..], self.is_child_ptr::<1>(), slot_1_payload);
                }
                replacement_node.set_child(
                    key_1[0],
                    TrieNodeODRc::new_in(child_node, self.alloc.clone()),
    ...
    fn subtract_from_slot_contents<const SLOT: usize>(
        &self,
        other: TaggedNodeRef<V, A>,
    ) -> AlgebraicResult<ValOrChildUnion<V, A>>
    where
        V: Clone + DistributiveLattice,
    {
        ...
        if let Some((onward_key, onward_node)) = follow_path(other, path) {
            if self.is_child_ptr::<SLOT>() {
                ...
                let difference = if onward_key.is_empty() {
                    ...
                    match onward_node.get_node_at_key(onward_key).into_option() {
                        Some(other_onward_node) => {
                            self_onward_link.psubtract_dyn(other_onward_node.as_tagged())
                        }
                ...
                difference.map(ValOrChildUnion::from)
            } else {
                debug_assert!(!onward_key.is_empty());
                let self_val = unsafe { self.val_in_slot::<SLOT>() };
                match onward_node.node_get_val(onward_key) {
                    Some(other_val) => self_val
                        .psubtract(other_val)
                        .map(|val| ValOrChildUnion::from(val)),
    ...
    fn restrict_slot_contents<const SLOT: usize>(
        &self,
        other: TaggedNodeRef<V, A>,
    ) -> AlgebraicResult<ValOrChildUnion<V, A>>
    where
        ...
        if self.is_used::<SLOT>() {
            ...
            if let Some((onward_key, onward_node)) = onward {
                if self.is_child_ptr::<SLOT>() {
                    ...
                    let restricted_node_result = if onward_key.is_empty() {
                        self_onward_link.as_tagged().prestrict_dyn(onward_node)
                    } else {
                        let other_onward_node = onward_node.get_node_at_key(onward_key);
                        self_onward_link
                            .as_tagged()
                            .prestrict_dyn(other_onward_node.as_tagged())
                    };
                    restricted_node_result.map(ValOrChildUnion::from)
                } else {
                    AlgebraicResult::None
                }
            } else {
    ...
    #[inline]
    fn combine_slot_results_into_node_result(
        &self,
        slot0_result: AlgebraicResult<ValOrChildUnion<V, A>>,
        slot1_result: AlgebraicResult<ValOrChildUnion<V, A>>,
        ...
        let new_node = self
            .clone_with_updated_payloads(slot0_payload, slot1_payload)
            .unwrap();
...
fn should_swap_keys(key0: &[u8], key1: &[u8]) -> bool {
    debug_assert!(!key0.is_empty());
    debug_assert!(!key1.is_empty());
...
>(
    ...
    b_key: &'a [u8],
    b: &LineListNode<V, A>,
) -> AlgebraicResult<(&'a [u8], ValOrChild<V, A>)> {
...
>(
    ...
    if b_key_len == overlap && b.is_child_ptr::<BSLOT>() && a_key_len > overlap {
        ...
        unsafe {
            intermediate_node.set_payload_owned::<0>(&a_key[overlap..], a_payload);
        }
        debug_assert!(validate_node(&intermediate_node));
        let intermediate_node = TrieNodeODRc::new_in(intermediate_node, a.alloc.clone());
        let joined = b_child.pjoin(&intermediate_node).unwrap_or_else(
    ...
    if a_key_len == overlap && a.is_child_ptr::<ASLOT>() && b_key_len > overlap {
        ...
        let mut intermediate_node = LineListNode::new_in(a.alloc.clone());
        unsafe {
            intermediate_node.set_payload_owned::<0>(&b_key[overlap..], b_payload);
        }
        debug_assert!(validate_node(&intermediate_node));
        let intermediate_node = TrieNodeODRc::new_in(intermediate_node, a.alloc.clone());
        let joined = a_child.pjoin(&intermediate_node).unwrap_or_else(
            |which_arg| match which_arg {
                0 => a_child.clone(),
                1 => intermediate_node,
                _ => unreachable!(),
    ...
    if overlap > 0 {
        ...
        if new_a_key[0] == new_b_key[0] {
            if a_key_len < b_key_len {
                unsafe {
                    new_node.set_payload_owned::<0>(new_a_key, a_payload);
                }
                unsafe {
                    new_node.set_payload_owned::<1>(new_b_key, b_payload);
                }
            } else {
                unsafe {
                    new_node.set_payload_owned::<0>(new_b_key, b_payload);
                }
                unsafe {
                    new_node.set_payload_owned::<1>(new_a_key, a_payload);
                }
            }
        } else {
            if new_a_key[0] < new_b_key[0] {
        ...
        AlgebraicResult::Element((
            &a_key[..overlap],
            ValOrChild::Child(TrieNodeODRc::new_in(new_node, a.alloc.clone())),
        ))
    } else {
        AlgebraicResult::None
    }
...
fn merge_list_nodes<V: Clone + Send + Sync + Lattice, A: Allocator>(
    ...
    let mut entries: [MaybeUninit<(&[u8], ValOrChild<V, A>)>; 4] = [
        MaybeUninit::uninit(),
        MaybeUninit::uninit(),
        MaybeUninit::uninit(),
        MaybeUninit::uninit(),
    ...
    match try_merge::<V, A, 0, 0>(self_key0, a, other_key0, b) {
        ...
        AlgebraicResult::Identity(mask) => {
            if mask & SELF_IDENT > 0 {
                ...
                entries[entry_cnt] =
                    MaybeUninit::new((other_key0, b.clone_payload::<0>().unwrap()));
            }
    ...
    match try_merge::<V, A, 0, 1>(self_key0, a, other_key1, b) {
        AlgebraicResult::Element(joined) => {
            entries[entry_cnt] = MaybeUninit::new(joined);
            entry_cnt += 1;
            debug_assert!(!used[0]); //If we create multiple joined entries from the same source, it's a bug somewhere
            used[0] = true;
            used[3] = true;
        }
        AlgebraicResult::Identity(mask) => {
            if mask & SELF_IDENT > 0 {
                ...
                debug_assert!(mask & COUNTER_IDENT > 0);
                entries[entry_cnt] =
                    MaybeUninit::new((other_key1, b.clone_payload::<1>().unwrap()));
            }
            identity_masks[entry_cnt] = mask;
            entry_cnt += 1;
            debug_assert!(!used[0]); //See above
            used[0] = true;
            used[3] = true;
        }
        AlgebraicResult::None => {}
    ...
    match try_merge::<V, A, 1, 1>(self_key1, a, other_key1, b) {
        AlgebraicResult::Element(joined) => {
            entries[entry_cnt] = MaybeUninit::new(joined);
            entry_cnt += 1;
            debug_assert!(!used[1]); //See above
            debug_assert!(!used[3]); //See above
            used[1] = true;
            used[3] = true;
        }
        AlgebraicResult::Identity(mask) => {
    ...
    if !used[0]
        && let Some(payload) = a.clone_payload::<0>()
    {
    ...
    if !used[1]
        && let Some(payload) = a.clone_payload::<1>()
    {
    ...
    if !used[2]
        && let Some(payload) = b.clone_payload::<0>()
    {
    ...
    if !used[3]
        && let Some(payload) = b.clone_payload::<1>()
    {
        ...
        match entry_cnt {
            1 => {
                if identity_masks[0] > 0 {
                    ...
                    unsafe {
                        joined_node.set_payload_owned::<0>(key0, payload0);
                    }
            ...
            2 => {
                ...
                if new_ident_mask > 0 {
                    ...
                    if should_swap_keys(key0, key1) {
                        unsafe {
                            joined_node.set_payload_owned::<0>(key1, payload1);
                        }
                        unsafe {
                            joined_node.set_payload_owned::<1>(key0, payload0);
                        }
                    } else {
                        unsafe {
                            joined_node.set_payload_owned::<0>(key0, payload0);
                        ...
                        unsafe {
                            joined_node.set_payload_owned::<1>(key1, payload1);
                        }
                    }
                    debug_assert!(validate_node(&joined_node));
                    return Ok(AlgebraicResult::Element(joined_node));
...
fn merge_into_list_nodes<V: Clone + Send + Sync + Lattice, A: Allocator>(
    target: &mut LineListNode<V, A>,
    other: &LineListNode<V, A>,
    ...
    match merge_list_nodes(target, other) {
        ...
        Err(AlgebraicResult::Element(new_dense_node)) => (
            AlgebraicStatus::Element,
            Err(TrieNodeODRc::new_in(new_dense_node, target.alloc.clone())),
        ),
...
fn follow_path<'a, 'k, V: Clone + Send + Sync, A: Allocator + 'a>(
    mut node: TaggedNodeRef<'a, V, A>,
    mut key: &'k [u8],
) -> Option<(&'k [u8], TaggedNodeRef<'a, V, A>)> {
...
impl<V: Clone + Send + Sync, A: Allocator> TrieNode<V, A> for LineListNode<V, A> {
    ...
    fn node_set_val(
        ...
        debug_assert!(!key.is_empty());
        self.set_payload_abstract::<false>(key, val.into())
            .map(|(result, created_subnode)| {
    ...
    fn node_create_dangling(&mut self, key: &[u8]) -> Result<(bool, bool), TrieNodeODRc<V, A>> {
        ...
        if !self.node_contains_partial_key(key) {
            self.set_payload_abstract::<true>(key, ValOrChildUnion::from(TrieNodeODRc::new_empty()))
                .map(|(result, created_subnode)| {
    ...
    fn node_remove_dangling(&mut self, key: &[u8]) -> usize {
        ...
        if self.is_used_child_0() && key0 == key {
            ...
            if child.as_tagged().node_is_empty() {
                let pruned_bytes = if !key1.is_empty() && key[0] == key1[0] {
                    key.len() - 1
                } else {
                    key.len()
                };
    ...
    fn first_child_from_key(&self, key: &[u8]) -> (Option<&[u8]>, Option<TaggedNodeRef<'_, V, A>>) {
        ...
        //Case 1
        if key.is_empty() {
            if !key1.is_empty() && key0[0] == key1[0] {
                if key0.len() == 1 && self.is_child_ptr::<0>() {
                    return (Some(key0), unsafe {
                        Some(self.child_in_slot::<0>().as_tagged())
                    });
                }
                if key1.len() == 1 && self.is_child_ptr::<1>() {
        ...
        if key.len() == 1 && key0.len() == 1 && key0[0] == key[0] {
            ...
            if key1.len() > 1 && key1[0] == key[0] {
                ...
                if self.is_child_ptr::<1>() {
                    return (Some(remaining_key), unsafe {
                        Some(self.child_in_slot::<1>().as_tagged())
                    });
                } else {
    ...
    fn prior_branch_key<'key>(&self, key: &'key [u8]) -> &'key [u8] {
        ...
        if key0.first() == key_byte && key1.first() == key_byte {
            &key[0..1]
        } else {
            &[]
        }
                ...
                if starts_with(key0, key) && starts_with(key1, common_key) {
                    ...
                    if key1_last_byte != key.last().unwrap() {
                        let sib_node = if key1.len() == key.len() && self.is_child_ptr::<1>() {
                            ...
                            debug_assert!({
                                sib_node.as_list().map(|sib_node| validate_node(sib_node));
                                true
                            });
                            Some(sib_node)
                        } else {
    ...
    fn get_node_at_key(&self, key: &[u8]) -> AbstractNodeRef<'_, V, A> {
        ...
        if key0.len() > key.len() && starts_with(key0, key) {
            ...
            if new_key.len() <= 7 {
                let ref_node = TinyRefNode::new_in(
                    self.is_child_ptr::<0>(),
                    new_key,
                    &self.val_or_child0,
                    self.alloc.clone(),
                ...
                unsafe {
                    new_node.set_payload_owned::<0>(new_key, payload);
                }
                debug_assert!(validate_node(&new_node));
                return AbstractNodeRef::OwnedRc(TrieNodeODRc::new_in(
                    new_node,
                    self.alloc.clone(),
                ));
            }
        ...
        match other.tag() {
            LINE_LIST_NODE_TAG => {
                ...
                match merge_list_nodes(self, other_list_node) {
                    Ok(joined_list_node) => {
                        joined_list_node.map(|node| TrieNodeODRc::new_in(node, self.alloc.clone()))
                    }
                    Err(joined_dense_node) => {
                        joined_dense_node.map(|node| TrieNodeODRc::new_in(node, self.alloc.clone()))
                    }
                }
            }
            DENSE_BYTE_NODE_TAG => {
        ...
        V: Lattice,
    {
        ...
        match other_node.tag() {
            ...
            DENSE_BYTE_NODE_TAG => {
                ...
                (
                    AlgebraicStatus::Element,
                    Err(TrieNodeODRc::new_in(new_node, self.alloc.clone())),
                )
            }
            #[cfg(feature = "bridge_nodes")]
            TaggedNodeRef::BridgeNode(_other_bridge_node) => {
        ...
        V: Lattice,
    {
        ...
        if !self.is_used::<1>() {
            ...
            if byte_cnt < key_len {
                ...
                debug_assert!(temp_node.is_child_ptr::<0>());
        ...
        if byte_cnt < key0_len && byte_cnt < key1_len {
            ...
            if key0[byte_cnt..] <= key1[byte_cnt..] {
                ...
                unsafe {
                    ...
                    let mut tmp_key_buf: [MaybeUninit<u8>; KEY_BYTES_CNT] =
                        [MaybeUninit::new(0); KEY_BYTES_CNT];
            ...
            temp_node.header =
                Self::header0(slot0_child, new_key0_len) | Self::header1(slot1_child, new_key1_len);
        ...
        let merged_payload =
            match merge_guts::<V, A, 0, 1>(overlap + 1, new_key0, &temp_node, new_key1, &temp_node)
            {
        ...
        V: Lattice,
    {
        ...
        pmeet_generic::<2, V, A, _>(self_payloads, other, |payloads| {
            ...
            let slot0_payload = payloads
                .get_mut(0)
                .and_then(core::mem::take)
                .map(|p| p.into());
            let slot1_payload = payloads
                .get_mut(1)
                .and_then(core::mem::take)
                .map(|p| p.into());
            let new_node = self
...
pub(crate) fn validate_node<V: Clone + Send + Sync, A: Allocator>(
    node: &LineListNode<V, A>,
) -> bool {
    let (key0, key1) = node.get_both_keys();

    //If a key is used it must be non-zero length
    if node.is_used::<0>() && key0.is_empty() || node.is_used::<1>() && key1.is_empty() {
    ...
    if key0.first() == key1.first() && key0.get(1) == key1.get(1) && key0.get(1).is_some() {
...
mod tests {
    ...
    fn get_recursive<'a, 'b, V: Clone + Send + Sync, A: Allocator + 'b>(
        key: &'a [u8],
        node: TaggedNodeRef<'b, V, A>,
    ) -> (&'a [u8], TaggedNodeRef<'b, V, A>, usize) {
    ...
    fn test_line_list_node() {
        ...
        #[cfg(feature = "slim_ptrs")]
        assert_eq!(
            core::mem::size_of::<LineListNode<[u8; 1024], GlobalAlloc>>(),
            64
        );
        #[cfg(not(feature = "slim_ptrs"))]
        assert_eq!(
            core::mem::size_of::<LineListNode<[u8; 1024], GlobalAlloc>>(),
            48
        ); //Shrunk to account for DynBox header
        ...
        assert_eq!(
            new_node.node_set_val("hello".as_bytes(), 42).map_err(|_| 0),
            Ok((None, false))
        );
        ...
        *new_node.node_get_val_mut("hello".as_bytes()).unwrap() = 123;
        ...
        assert_eq!(
            new_node
                .node_set_val("hello".as_bytes(), [0, 1, 2, 3])
                .map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_get_val("hello".as_bytes()),
            Some(&[0, 1, 2, 3])
        );
        *new_node.node_get_val_mut("hello".as_bytes()).unwrap() = [3, 2, 1, 0];
        ...
        assert_eq!(
            new_node
                .node_set_val("goodbye".as_bytes(), 24)
                .map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(new_node.node_get_val("hello".as_bytes()), Some(&42));
    ...
    fn test_line_list_node_key_and_value_collision() {
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val("a".as_bytes(), 42).map_err(|_| 0),
            Ok((None, false))
        );
        ...
        assert_eq!(
            child_node
                .node_set_val("hello".as_bytes(), 24)
                .map_err(|_| 0),
            Ok((None, false))
        );
        ...
        unsafe {
            new_node.set_child_1(
                "a".as_bytes(),
                TrieNodeODRc::new_in(child_node, global_alloc()),
            );
        }
    ...
    fn test_line_list_node_shared_prefixes_empty() {
        ...
        assert_eq!(
            new_node
                .node_set_val("my name is".as_bytes(), 42)
                .map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node
                .node_set_val("my name is billy".as_bytes(), 24)
                .map_err(|_| 0),
            Ok((None, true))
        );
    ...
    fn test_line_list_node_overflow_keys() {
        ...
        assert_eq!(
            new_node.node_set_val(LONG_KEY, 24).map_err(|_| 0),
            Ok((None, true))
        );
        assert_eq!(new_node.node_get_val("hello".as_bytes()), Some(&42));
    ...
    fn test_line_list_shorten_key_releases_slot_1_availability() {
        ...
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val(&full_key, 24).map_err(|_| 0),
            Ok((None, false))
        );
        ...
        assert_eq!(
            new_node.node_set_val(b"z", 42).map_err(|_| 0),
            Ok((None, false))
        );
    ...
    fn test_line_list_drop_head_releases_slot_1_availability() {
        ...
        let mut shortened = new_node
            .drop_head_dyn(drop_bytes)
            .unwrap()
            .as_tagged()
            .as_list()
            .unwrap()
        ...
        assert_eq!(
            shortened.node_set_val(b"z", 42).map_err(|_| 0),
            Ok((None, false))
        );
    ...
    fn test_line_list_node_shared_prefixes_slot_0() {
        ...
        assert_eq!(
            new_node
                .node_set_val("I'm billy".as_bytes(), 42)
                .map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node
                .node_set_val("slot1".as_bytes(), 123)
                .map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node
                .node_set_val("I'm johnny".as_bytes(), 24)
                .map_err(|_| 0),
            Ok((None, true))
        );
    ...
    fn test_line_list_node_shared_prefixes_slot_1() {
        ...
        assert_eq!(
            new_node
                .node_set_val("slot0".as_bytes(), 123)
                .map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
    ...
    fn test_line_list_node_replacement() {
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val("apple".as_bytes(), 1).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_set_val("banana".as_bytes(), 2).map_err(|_| 0),
            Ok((None, false))
        );
        ...
        let (bytes_used, child_node) = replacement_node
            .as_tagged()
            .node_get_child("apple".as_bytes())
            .unwrap();
        let child_node = child_node.as_tagged();
        assert_eq!(bytes_used, 1);
        ...
        let (bytes_used, child_node) = replacement_node
            .as_tagged()
            .node_get_child("banana".as_bytes())
            .unwrap();
        let child_node = child_node.as_tagged();
        assert_eq!(bytes_used, 1);
        ...
        let (bytes_used, child_node) = replacement_node
            .as_tagged()
            .node_get_child("carrot".as_bytes())
            .unwrap();
        let child_node = child_node.as_tagged();
        assert_eq!(bytes_used, 1);
    ...
    fn test_line_list_join_1_single_values_no_conflict() {
        ...
        a.node_set_val("apple".as_bytes(), 0)
            .unwrap_or_else(|_| panic!());
        let mut b = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        b.node_set_val("banana".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());

        let joined_result = a.pjoin_dyn(b.as_tagged());
        let join_list_node = joined_result
            .as_ref()
            .map(|joined| joined.as_tagged().as_list().unwrap())
            .unwrap([&a, &b]);
    ...
    fn test_line_list_join_2_single_values_joined() {
        let mut a = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        a.node_set_val("apple".as_bytes(), 42)
            .unwrap_or_else(|_| panic!());
        let mut b = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        b.node_set_val("apple".as_bytes(), 24)
            .unwrap_or_else(|_| panic!());
    ...
    fn test_line_list_join_3_single_values_prefix_joined() {
        ...
        a.node_set_val("apple".as_bytes(), 42)
            .unwrap_or_else(|_| panic!());
        let mut b = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        b.node_set_val("apricot".as_bytes(), 24)
            .unwrap_or_else(|_| panic!());
        let joined_result = a.pjoin_dyn(b.as_tagged());
        let join_list_node = joined_result
            .as_ref()
        ...
        let (remaining_key, child_node, _) =
            get_recursive("apple".as_bytes(), join_list_node.as_tagged());
        assert_eq!(child_node.node_get_val(remaining_key), Some(&42));

        let (remaining_key, child_node, _) =
            get_recursive("apricot".as_bytes(), join_list_node.as_tagged());
        assert_eq!(child_node.node_get_val(remaining_key), Some(&24));
    ...
    fn test_line_list_join_4_bytes() {
        let mut a = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        a.node_set_val("0".as_bytes(), 0)
            .unwrap_or_else(|_| panic!());
        a.node_set_val("1".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());
        let mut b = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        b.node_set_val("1".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());
        b.node_set_val("0".as_bytes(), 0)
            .unwrap_or_else(|_| panic!());
    ...
    fn test_line_list_join_6_bytes() {
        ...
        a.node_set_val("1".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());
        let mut b = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        b.node_set_val("2".as_bytes(), 2)
            .unwrap_or_else(|_| panic!());
        b.node_set_val("1".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());

        let joined_result = a.pjoin_dyn(b.as_tagged());
        let joined_node = joined_result
            .as_ref()
            .map(|joined| joined.as_tagged())
            .unwrap([a.as_tagged(), b.as_tagged()]);
    ...
    fn test_line_list_join_8_common_byte_prefix() {
        let mut a = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        a.node_set_val("0a".as_bytes(), 0)
            .unwrap_or_else(|_| panic!());
        a.node_set_val("1a".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());
        let mut b = LineListNode::<u64, GlobalAlloc>::new_in(global_alloc());
        b.node_set_val("1b".as_bytes(), 1)
            .unwrap_or_else(|_| panic!());
        b.node_set_val("2b".as_bytes(), 2)
            .unwrap_or_else(|_| panic!());
        debug_assert!(validate_node(&a));
        debug_assert!(validate_node(&b));
    ...
    fn test_line_list_siblings_and_children() {
        ...
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val(b"albatross", 42).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node
                .node_set_val(b"brown-winged whistling thrush (Myophonus castaneus)", 42)
                .map_err(|_| 0),
            Ok((None, true))
        );
        debug_assert!(validate_node(&new_node));
        ...
        assert_eq!(
            new_node
                .get_sibling_of_child(b"brown-winged whistling thrush", true)
                .0,
            None
        );
        ...
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val(b"a", 42).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_set_val(b"albatross", 24).map_err(|_| 0),
            Ok((None, false))
        );
        debug_assert!(validate_node(&new_node));
        ...
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val("al".as_bytes(), 24).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node
                .node_set_val("albatross".as_bytes(), 42)
                .map_err(|_| 0),
            Ok((None, true))
        );
        debug_assert!(validate_node(&new_node));
        ...
        assert_eq!(
            new_node.node_set_val("a".as_bytes(), 24).map_err(|_| 0),
            Ok((None, false))
        );
        debug_assert!(validate_node(&new_node));
        assert_eq!(new_node.count_branches(b""), 1);
