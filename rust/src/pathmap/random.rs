...
mod tests {
    ...
    fn fixed_length() {
        ...
        let res = [
            "aaa", "aac", "bba", "bdd", "cbb", "cbd", "dab", "dac", "dca",
        ];
        trie.iter()
            .zip(res)
            .for_each(|(x, y)| assert_eq!(x.0.as_slice(), y.as_bytes()));
    }
    ...
    fn fair_trie_value() {
        ...
        let hist = Histogram::from_iter(
            stv.sample_iter(rng)
                .map(|(_, v)| v)
                .take(pairs.len() * SAMPLES),
        );
    ...
    fn resample_trie() {
        ...
        let submaps = Collected {
            d: Product2 {
                ...
                dy: lengths,
                f: |(path, v), l| {
                    if v.is_none() && path.len() == l.round() as usize {
                        Some(path)
                    } else {
                        None
                    }
