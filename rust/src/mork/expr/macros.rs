...
macro_rules! destruct {
    ...
    ($expr:expr, { $var:ident : $ty:ty }, $good:expr, $err:ident => $bad:expr) => {
        crate::mork::expr::destruct!($expr, { $var: $ty } , _length => $good, $err => $bad);
    };
    ($expr:expr, { $var:ident : $ty:ty }, $length:ident => $good:expr, $err:ident => $bad:expr) => {
        use crate::mork::expr::macros::{DeserializableExpr};
    ...
    ($expr:expr, ( $($pattern:tt)* ), $good:expr, $err:ident => $bad:expr) => {
        crate::mork::expr::destruct!($expr, ( $($pattern)* ), _length => $good, $err => $bad)
    };
    ($expr:expr, ( $($pattern:tt)* ), $length:ident => $good:expr, $err:ident => $bad:expr) => {
        {
            use ::core::convert::TryFrom;
            use crate::mork::expr::{Tag, Expr, byte_item, parse};
            use crate::mork::expr::macros::{DeserializableExpr};
            let mut length = 0;
            let rv = 'ret: loop {
                unsafe {
                    let mut offset = 0;
                    crate::mork::expr::destruct!(@chomp, 'ret, $expr, offset, ( $($pattern)* ) );
                    length = offset;
                    break 'ret Ok(crate::mork::expr::destruct!(@vars, $($pattern)*))
                }
            };
            match rv {
                Ok(crate::mork::expr::destruct!(@vars, $($pattern)*)) => {
                    let $length = length;
                    $good
                }
                Err($err) => $bad,
    ...
    (@length, $( $tts:tt )*) => { {
        ...
        // $( ${ignore($tts)} len += 1; )*
        $( crate::mork::expr::destruct!(@ignore, $tts); len += 1; )*
        len
    } };
    ...
    (@vars, ( $( $head:tt )* ) $( $rest:tt )*) => {
        (
            crate::mork::expr::destruct!(@vars, $( $head )*),
            crate::mork::expr::destruct!(@vars, $( $rest )*),
        )
    };
    ...
    (@vars, { $var:ident : $ty:ty } $( $rest:tt )*) => {
        ($var, crate::mork::expr::destruct!(@vars, $( $rest )*))
    };
    ...
    (@chomp, $label:lifetime, $expr:expr, $offset:ident, ( $( $head:tt )* ) $( $rest:tt )*) => {
        let arity = crate::mork::expr::destruct!(@length, $( $head )*);
        ...
        $offset += 1;
        crate::mork::expr::destruct!(@chomp, $label, $expr, $offset, $( $head )*);
        crate::mork::expr::destruct!(@chomp, $label, $expr, $offset, $( $rest )*);
    };
...
impl SerializableExpr for Expr {
    fn size(&self) -> usize {
        self.span().len()
    }
    fn serialize<W: std::io::Write>(&self, buf: &mut W) -> Result<(), std::io::Error> {
