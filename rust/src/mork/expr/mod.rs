...
use std::{
    fmt::{Debug, Formatter, Write, format},
    mem,
...
pub fn byte_item(b: u8) -> Tag {
    if b == 0b1100_0000 {
        ...
        return Tag::SymbolSize(b & 0b0011_1111);
    } else if (b & 0b1100_0000) == 0b1000_0000 {
        return Tag::VarRef(b & 0b0011_1111);
    } else if (b & 0b1100_0000) == 0b0000_0000 {
        return Tag::Arity(b & 0b0011_1111);
    } else {
        panic!("reserved {}", b)
    }
}
...
pub const fn maybe_byte_item(b: u8) -> Result<Tag, u8> {
    if b == 0b1100_0000 {
        return Ok(Tag::NewVar);
    } else if (b & 0b1100_0000) == 0b1100_0000 {
        return Ok(Tag::SymbolSize(b & 0b0011_1111));
    } else if (b & 0b1100_0000) == 0b1000_0000 {
        return Ok(Tag::VarRef(b & 0b0011_1111));
    } else if (b & 0b1100_0000) == 0b0000_0000 {
        return Ok(Tag::Arity(b & 0b0011_1111));
    } else {
        return Err(b);
    }
}
...
macro_rules! traverse {
    ($t1:ty, $t2:ty, $x:expr, $new_var:expr, $var_ref:expr, $symbol:expr, $zero:expr, $add:expr, $finalize:expr) => {{
        ...
        impl Traversal<$t1, $t2> for AnonTraversal {
            #[inline(always)]
            fn new_var(&mut self, offset: usize) -> $t2 {
                ($new_var)(offset)
            }
            #[inline(always)]
            fn var_ref(&mut self, offset: usize, i: u8) -> $t2 {
                ($var_ref)(offset, i)
            }
            #[inline(always)]
            fn symbol(&mut self, offset: usize, s: &[u8]) -> $t2 {
                ($symbol)(offset, s)
            }
            #[inline(always)]
            fn zero(&mut self, offset: usize, a: u8) -> $t1 {
                ($zero)(offset, a)
            }
            #[inline(always)]
            fn add(&mut self, offset: usize, acc: $t1, sub: $t2) -> $t1 {
                ($add)(offset, acc, sub)
            }
            #[inline(always)]
            fn finalize(&mut self, offset: usize, acc: $t1) -> $t2 {
                ($finalize)(offset, acc)
            }
        }
    ...
    ($t1:ty, $t2:ty, $t3:ty, $x:expr, $v0:expr, $new_var:expr, $var_ref:expr, $symbol:expr, $zero:expr, $add:expr, $finalize:expr) => {{
        ...
        let value = 'putting: loop {
            let mut value = match unsafe { byte_item(*$x.ptr.byte_add(j)) } {
                ...
                Tag::SymbolSize(s) => {
                    let slice =
                        unsafe { &*slice_from_raw_parts($x.ptr.byte_add(j + 1), s as usize) };
...
impl Expr {
    pub fn symbol(self) -> Option<*const [u8]> {
        unsafe {
            if let Tag::SymbolSize(n) = byte_item(*self.ptr) {
                Some(slice_from_raw_parts(self.ptr.offset(1), n as usize))
            } else {
                None
            }
        }
    ...
    pub fn arity(self) -> Option<u8> {
        unsafe {
            if let Tag::Arity(n) = byte_item(*self.ptr) {
                Some(n)
            } else {
                None
            }
    ...
    pub fn forward_references(self, at: u8) -> usize {
        traverseh!(
            ...
            u64,
            self,
            if at > 0 { (!0u64) >> (64 - at) } else { 0 },
            |c: &mut u64, _| {
                *c |= 1u64 << ((*c).trailing_ones());
                0
            },
            |c: &mut u64, _, r| if (1u64 << r) & *c == 0 {
                *c |= 1u64 << r;
                1
            } else {
                0
    ...
    pub fn max_arity(self) -> Option<u8> {
        traverse!(
            ...
            |_, _| None,
            |_, a| a,
            |_, x, y: Option<u8>| u8::max(x, y.unwrap_or(0)),
            |_, x| Some(x)
        )
    }
    ...
    pub fn difference_under<F: Fn(Expr, Expr) -> bool>(
        self,
        other: Expr,
        under: F,
    ) -> Option<usize> {
    ...
    pub fn substitute(self, substitutions: &[Expr], oz: &mut ExprZipper) -> *const [u8] {
        ...
        loop {
            match ez.tag() {
                ...
                Tag::VarRef(r) => match unsafe { substitutions[r as usize].span().as_ref() } {
                    None => {
                        oz.write_var_ref(r);
                        oz.loc += 1;
                    }
                ...
                Tag::SymbolSize(s) => {
                    oz.write_move(unsafe {
                        slice_from_raw_parts(ez.root.ptr.byte_add(ez.loc), s as usize + 1)
                            .as_ref()
                            .unwrap()
                    });
                }
    ...
    pub fn substitute_one_de_bruijn(
        self,
        idx: u8,
        substitution: Expr,
        oz: &mut ExprZipper,
    ...
    pub fn substitute_de_bruijn_ivc(
        self,
        substitutions: &[Expr],
        oz: &mut ExprZipper,
        var_count: &mut usize,
            ...
            match ez.tag() {
                Tag::NewVar => {
                    ...
                    *var_count += 1;
                    ...
                    for j in *var_count..additions.len() {
                        additions[j] += nvars;
                    }
                }
                Tag::VarRef(r) => {
    ...
    fn bind(self, n: u8, oz: &mut ExprZipper) -> *const [u8] {
        ...
        loop {
            match ez.tag() {
                Tag::NewVar => {
                    oz.write_var_ref(n + var_count);
                    oz.loc += 1;
                    var_count += 1;
                }
                Tag::VarRef(i) => {
                    oz.write_var_ref(n + i);
                    oz.loc += 1; // good
                }
                Tag::SymbolSize(s) => {
                    oz.write_move(unsafe {
    ...
    pub fn unbind(self, oz: &mut ExprZipper) -> *const [u8] {
        ...
        loop {
            match ez.tag() {
                ...
                Tag::VarRef(i) => {
                    if (i as usize) < nvars || bound[i as usize] != 255 {
                        oz.write_var_ref(bound[i as usize]);
                        oz.loc += 1;
                    } else {
                        oz.write_new_var();
                        bound[i as usize] = nvars as u8;
                        nvars += 1;
                        oz.loc += 1;
                    }
                }
    ...
    pub fn transformData(
        ...
        match pattern.extract_data(&mut ez) {
            Ok(ref bindings) => {
                template.substitute(bindings, oz);
                Ok(())
            }
            Err(e) => Err(e),
        }
    ...
    pub fn transformed(self, template: Expr, pattern: Expr) -> Result<Expr, ExtractFailure> {
        ...
        let o = Expr {
            ptr: vec![0; 512].leak().as_mut_ptr(),
        };
        let x = Expr {
            ptr: transformation.as_mut_ptr(),
        };
        let y = Expr {
            ptr: data.as_mut_ptr(),
        };
    ...
    pub fn extract_data(self, iz: &mut ExprZipper) -> Result<Vec<Expr>, ExtractFailure> {
        ...
        loop {
            match (ez.tag(), iz.tag()) {
                ...
                (Tag::NewVar, Tag::SymbolSize(_)) => {
                    bindings.push(iz.subexpr());
                    iz.next();
                    if !ez.next() {
                        return Ok(bindings);
                    };
                }
                (Tag::NewVar, Tag::Arity(_a)) => {
                    bindings.push(iz.subexpr());
                ...
                (Tag::VarRef(i), Tag::SymbolSize(s)) => {
                    if let Tag::SymbolSize(s_) = unsafe { byte_item(*bindings[i as usize].ptr) } {
                        if s != s_ {
                            return Err(RefSymbolEarlyMismatch(i, s, s_));
                        }
                        let aslice = unsafe {
                            &*slice_from_raw_parts(
                                bindings[i as usize].ptr.byte_add(1),
                                s_ as usize,
                            )
                        };
                        let bslice = unsafe {
                            &*slice_from_raw_parts(iz.subexpr().ptr.byte_add(1), s as usize)
                        };
                        if aslice != bslice {
                            return Err(RefSymbolMismatch(i, aslice.to_vec(), bslice.to_vec()));
                        }
                        iz.next();
                        if !ez.next() {
                            return Ok(bindings);
                ...
                (Tag::VarRef(i), Tag::Arity(s)) => {
                    ...
                    if let Tag::Arity(s_) = unsafe { byte_item(*bindings[i as usize].ptr) } {
                        if s != s_ {
                            return Err(RefExprEarlyMismatch(i, s, s_));
                        }
                        ...
                        if aslice != bslice {
                            return Err(RefExprMismatch(i, aslice.to_vec(), bslice.to_vec()));
                        }
                        iz.next();
                        if !ez.next() {
                            return Ok(bindings);
                ...
                (Tag::SymbolSize(a), Tag::SymbolSize(b)) => {
                    if a != b {
                        return Err(SymbolEarlyMismatch(a, b));
                    }
                    let aslice =
                        unsafe { &*slice_from_raw_parts(ez.subexpr().ptr.byte_add(1), a as usize) };
                    let bslice =
                        unsafe { &*slice_from_raw_parts(iz.subexpr().ptr.byte_add(1), b as usize) };
                    if aslice != bslice {
                        return Err(SymbolMismatch(aslice.to_vec(), bslice.to_vec()));
                    }
                    iz.next();
                    if !ez.next() {
                        return Ok(bindings);
                ...
                (Tag::Arity(a), Tag::Arity(b)) => {
                    if a != b {
                        return Err(ExprEarlyMismatch(a, b));
                    }
                    iz.next();
                    if !ez.next() {
                        return Ok(bindings);
    ...
    pub fn substitute_symbols<F: for<'a> FnMut(&'a [u8]) -> &'a [u8]>(
        self,
        oz: &mut ExprZipper,
        mut subst: F,
    ) -> *mut [u8] {
    ...
    pub fn string(&self) -> String {
        let mut traversal = DebugTraversal {
            string: String::new(),
            transient: false,
        };
    ...
    #[inline(never)]
    pub fn serialize<Target: std::io::Write, F: for<'a> Fn(&'a [u8]) -> &'a str>(
        &self,
        t: &mut Target,
        map_symbol: F,
    ) -> () {
        let mut traversal = SerializerTraversal {
            out: t,
    ...
    pub const VARNAMES: [&'static str; 64] = [
        "$a", "$b", "$c", "$d", "$e", "$f", "$g", "$h", "$i", "$j", "$x10", "$x11", "$x12", "$x13",
    ...
    >(
        ...
        let mut traversal = SerializerTraversal2 {
            out: t,
            map_symbol: map_symbol,
    ...
    >(
        ...
        let mut traversal = SerializerTraversalHighlights {
            out: t,
            map_symbol: map_symbol,
            map_variable: map_variable,
            transient: false,
...
pub struct PairTraversal<A1, A2, R1, R2, T1, T2> {
    t1: T1,
    t2: T2,
    pd: std::marker::PhantomData<(A1, A2, R1, R2)>,
}

impl<A1, A2, R1, R2, T1: Traversal<A1, R1>, T2: Traversal<A2, R2>> Traversal<(A1, A2), (R1, R2)>
    for PairTraversal<A1, A2, R1, R2, T1, T2>
{
    fn new_var(&mut self, offset: usize) -> (R1, R2) {
        (self.t1.new_var(offset), self.t2.new_var(offset))
    }
    fn var_ref(&mut self, offset: usize, i: u8) -> (R1, R2) {
        (self.t1.var_ref(offset, i), self.t2.var_ref(offset, i))
    }
    fn symbol(&mut self, offset: usize, s: &[u8]) -> (R1, R2) {
        (self.t1.symbol(offset, s), self.t2.symbol(offset, s))
    }
    fn zero(&mut self, offset: usize, a: u8) -> (A1, A2) {
        (self.t1.zero(offset, a), self.t2.zero(offset, a))
    }
    fn add(&mut self, offset: usize, acc: (A1, A2), sub: (R1, R2)) -> (A1, A2) {
        (
            self.t1.add(offset, acc.0, sub.0),
            self.t2.add(offset, acc.1, sub.1),
        )
    }
    fn finalize(&mut self, offset: usize, acc: (A1, A2)) -> (R1, R2) {
        (
            self.t1.finalize(offset, acc.0),
            self.t2.finalize(offset, acc.1),
        )
    }
}
...
fn match2<
    F: FnMut(&mut T1, Expr, usize, &mut T2, Expr, usize),
    A1,
    R1,
    T1: Traversal<A1, R1>,
    A2,
...
>(
    ...
    match unsafe {
        (
            byte_item(*e1.ptr.byte_add(i1)),
            byte_item(*e2.ptr.byte_add(i2)),
        )
    } {
        (b1 @ (Tag::NewVar | Tag::VarRef(_)), _) => {
            hole(t1, e1, i1, t2, e2, i2);
            let r1 = if let Tag::VarRef(k1) = b1 {
                t1.var_ref(i1, k1)
            } else {
                t1.new_var(i1)
            };
        ...
        (_, b2 @ (Tag::NewVar | Tag::VarRef(_))) => {
            hole(t1, e1, i1, t2, e2, i2);
            let r2 = if let Tag::VarRef(k2) = b2 {
                t2.var_ref(i2, k2)
            } else {
                t2.new_var(i2)
            };
...
pub fn execute_loop_truncated<A, R, T: Traversal<A, R>>(
    t: &mut T,
    e: Expr,
    m: usize,
) -> Result<(usize, R), (Vec<(u8, A)>, u8)> {
    let mut stack: Vec<(u8, A)> = Vec::with_capacity(8);
    let mut j = 0;
    'putting: loop {
        if j == m {
            return Err((stack, 0u8));
        }
        let mut value = match unsafe { byte_item(*e.ptr.byte_add(j)) } {
            Tag::NewVar => {
                j += 1;
        ...
        'popping: loop {
            match stack.last_mut() {
                ...
                Some(&mut (ref mut k, ref mut acc)) => unsafe {
                    std::ptr::write(k, std::ptr::read(k).wrapping_sub(1));
                    std::ptr::write(acc, t.add(j, std::ptr::read(acc), value));
                    if std::ptr::read(k) != 0 {
                        continue 'putting;
...
impl Traversal<(), ()> for DebugTraversal {
    #[inline(always)]
    fn new_var(&mut self, offset: usize) -> () {
        if self.transient {
            self.string.push(' ');
        };
        self.string.push('$');
    ...
    #[inline(always)]
    fn var_ref(&mut self, offset: usize, i: u8) -> () {
        if self.transient {
            self.string.push(' ');
        };
        self.string.push('_');
        self.string.push_str((i as u16 + 1).to_string().as_str());
    }
    #[inline(always)]
    fn symbol(&mut self, offset: usize, s: &[u8]) -> () {
        match std::str::from_utf8(s) {
            ...
            Err(_) => {
                ...
                for b in s {
                    self.string.push_str(format!("\\x{:x}", b).as_str());
                }
            }
        }
    }
    #[inline(always)]
    fn zero(&mut self, offset: usize, a: u8) -> () {
        if self.transient {
            self.string.push(' ');
        };
        self.string.push('(');
    ...
    #[inline(always)]
    fn add(&mut self, offset: usize, acc: (), sub: ()) -> () {
        self.transient = true;
    }
    #[inline(always)]
    fn finalize(&mut self, offset: usize, acc: ()) -> () {
        self.string.push(')');
    }
}
...
struct SerializerTraversal<'a, Target: std::io::Write, F: for<'b> Fn(&'b [u8]) -> &'b str> {
    out: &'a mut Target,
    map_symbol: F,
    transient: bool,
}
#[allow(unused_variables, unused_must_use)]
impl<Target: std::io::Write, F: for<'b> Fn(&'b [u8]) -> &'b str> Traversal<(), ()>
    for SerializerTraversal<'_, Target, F>
{
    #[inline(always)]
    fn new_var(&mut self, offset: usize) -> () {
        if self.transient {
    ...
    fn var_ref(&mut self, offset: usize, i: u8) -> () {
        ...
        self.out.write_all("_".as_bytes());
        self.out.write_all((i as u16 + 1).to_string().as_bytes());
    }
    #[inline(always)]
    fn symbol(&mut self, offset: usize, s: &[u8]) -> () {
        if self.transient {
...
struct SerializerTraversal2<
    'a,
    Target: std::io::Write,
    F: for<'b> Fn(&'b [u8]) -> &'b str,
    G: Fn(u8, bool) -> &'static str,
> {
    out: &'a mut Target,
    map_symbol: F,
...
#[allow(unused_variables, unused_must_use)]
impl<Target: std::io::Write, F: for<'b> Fn(&'b [u8]) -> &'b str, G: Fn(u8, bool) -> &'static str>
    Traversal<(), ()> for SerializerTraversal2<'_, Target, F, G>
{
    #[inline(always)]
    fn new_var(&mut self, offset: usize) -> () {
        if self.transient {
        ...
        self.out
            .write_all((self.map_variable)(self.n, true).as_bytes());
        self.n += 1;
    }
    #[inline(always)]
    fn var_ref(&mut self, offset: usize, i: u8) -> () {
...
struct SerializerTraversalHighlights<
    'a,
    't,
    Target: std::io::Write,
    F: for<'b> Fn(&'b [u8]) -> &'b str,
...
> {
    ...
    transient: bool,
    n: u8,
    targets: &'t [(usize, &'static str, &'static str)],
}
#[allow(unused_variables, unused_must_use)]
impl<Target: std::io::Write, F: for<'b> Fn(&'b [u8]) -> &'b str, G: Fn(u8, bool) -> &'static str>
    Traversal<Option<&'static str>, ()> for SerializerTraversalHighlights<'_, '_, Target, F, G>
{
    #[inline(always)]
    fn new_var(&mut self, offset: usize) -> () {
        if self.transient {
        ...
        if offset == self.targets[0].0 {
            self.out.write_all(self.targets[0].1.as_bytes());
        }
        self.out
            .write_all((self.map_variable)(self.n, true).as_bytes());
        if offset == self.targets[0].0 {
            self.out.write_all(self.targets[0].2.as_bytes());
    ...
    #[inline(always)]
    fn zero(&mut self, offset: usize, a: u8) -> Option<&'static str> {
        if self.transient {
            self.out.write_all(" ".as_bytes());
        };
        if offset == self.targets[0].0 {
    ...
    #[inline(always)]
    fn add(&mut self, offset: usize, acc: Option<&'static str>, sub: ()) -> Option<&'static str> {
        self.transient = true;
        acc
    }
    #[inline(always)]
    fn finalize(&mut self, offset: usize, acc: Option<&'static str>) -> () {
        self.out.write_all(")".as_bytes());
        if let Some(end) = acc {
...
struct HashTraversal<
    H: Hasher,
    F: for<'b> Fn(&'b [u8], &'b mut H),
    G: for<'b> Fn(u8, bool, &'b mut H),
> {
    hasher: H,
    map_symbol: F,
    map_variable: G,
...
impl<H: Hasher, F: for<'b> Fn(&'b [u8], &'b mut H), G: for<'b> Fn(u8, bool, &'b mut H)>
    Traversal<(), ()> for HashTraversal<H, F, G>
{
    #[inline(always)]
    fn new_var(&mut self, offset: usize) -> () {
...
impl ExprZipper {
    ...
    pub fn item(&self) -> Result<Tag, &[u8]> {
        let tag = self.tag();
        if let Tag::SymbolSize(n) = tag {
            return unsafe {
                Err(&*slice_from_raw_parts(
                    self.root.ptr.byte_add(self.loc + 1),
                    n as usize,
                ))
            };
        } else {
    ...
    pub fn subexpr(&self) -> Expr {
        unsafe {
            Expr {
                ptr: self.root.ptr.byte_add(self.loc),
            }
        }
    }
    #[inline]
    pub fn span(&self) -> &[u8] {
        unsafe { &*slice_from_raw_parts(self.root.ptr, self.loc) }
    }
    ...
    pub fn item_str(&self) -> String {
        match self.item() {
            ...
            Err(s) => match std::str::from_utf8(s) {
                Ok(string) => {
                    format!("{}", string)
                }
                Err(_) => {
            ...
            Some(&mut Breadcrumb {
                ...
                if *s < a {
                    *s += 1;
                    ...
                    self.loc += if let Tag::SymbolSize(n) = self.tag() {
                        n as usize + 1
                    } else {
                        1
                    };
                    ...
                    if let Tag::Arity(a) = self.tag() {
                        self.trace.push(Breadcrumb {
                            parent: self.loc as u32,
                            arity: a,
                            seen: 0,
                        })
                    }
    ...
    pub fn parent(&mut self) -> bool {
        let Some(Breadcrumb { parent: p, .. }) = self.trace.last() else {
            return false;
        };
        self.loc = *p as usize;
        self.trace.pop();
    ...
    pub fn next_descendant(&mut self, to: i32, offset: usize) -> bool {
        let (base, _backup) = if to < 0 {
            ...
            (
                self.trace[(to - 1) as usize].parent,
                self.trace[(to - 1) as usize],
            )
        } else {
            (0, self.trace[0])
        };
    ...
    pub fn traverse(&self, i: usize) -> usize {
        match unsafe { maybe_byte_item(*self.root.ptr.byte_add(self.loc + i)) } {
            ...
            Ok(Tag::SymbolSize(s)) => {
                let slice = unsafe {
                    &*slice_from_raw_parts(self.root.ptr.byte_add(self.loc + i + 1), s as usize)
                };
    ...
    pub fn finish_span(&self) -> *const [u8] {
        let size = self.loc
            + match self.tag() {
                ...
                Tag::Arity(0) => 1,
                Tag::Arity(_a) => {
                    unreachable!() /* expression can't end in non-zero expression */
                }
            };
                    ...
                    Tag::SymbolSize(s) => {
                        ...
                        if i + (s as usize) > bytes.len() {
                            ...
                            for j in i..bytes.len() {
                                ...
                                if (symbol_byte.is_ascii_graphic() || symbol_byte == b' ')
                                    && symbol_byte != b'\\'
                                {
                            ...
                            result.push_str(
                                format!("<truncated {}>", (i + (s as usize)) - bytes.len())
                                    .as_str(),
                            );
                            i += s as usize;
                            break;
...
impl ExprEnv {
    ...
    pub fn subsexpr(&self) -> Expr {
        Expr {
            ptr: unsafe { self.base.ptr.add(self.offset as usize) },
        }
    }

    pub fn show(&self) -> String {
        let mut v = vec![];
        self.base.serialize_highlight(
            &mut v,
            |x| std::str::from_utf8(x).unwrap_or_else(|_| format!("{:?}", x).leak()),
    ...
    fn same_functor(&self, other: &Self) -> bool {
        unsafe {
            if self.n == other.n && self.offset == other.offset {
                return true;
            }
            let lhs = self.subsexpr();
            let lprefix = lhs
                .prefix()
                .unwrap_or_else(|x| slice_from_raw_parts(x as *const _, x.len() + 1))
                .as_ref()
                .unwrap();
        ...
        unsafe {
            match byte_item(*self.subsexpr().ptr) {
                ...
                Tag::Arity(k) => {
                    ...
                    for sk in 0..k {
                        let (se_c, _, se_offset) = traverseh!(
                            (),
                            (),
                            u8,
                            env.subsexpr(),
...
pub(crate) const APPLY_DEPTH: u32 = 64;
        ...
        match (dt1.var_opt(), dt2.var_opt()) {
            (None, None) => {
                ...
                if let Err((o1, o2)) = match2(
                    ...
                    dt2.subsexpr(),
                    0,
                    &mut |_ts1, e1, i1, _ts2, e2, i2| step!(push _ts1.ee.offset(i1 as u32), _ts2.ee.offset(i2 as u32)),
                ) {
                    if PRINT_DEBUG {
                        println!(
                            "diff {} @ {}  != {} @ {}",
                            dt1.offset(o1 as u32).show(),
                            o1,
                            dt2.offset(o2 as u32).show(),
                            o2
                        );
                    }
            ...
            (Some(vx), ov) => {
                ...
                if step!(occurs vx, dt2) {
                    return Err(UnificationFailure::Occurs(vx, dt2));
                }
            ...
            (ov, Some(vy)) => {
                ...
                if step!(occurs vy, dt1) {
                    return Err(UnificationFailure::Occurs(vy, dt1));
                }
            ...
            traverseh!(
                ...
                |b: &mut bool, o, s: &[u8]| {
                    ...
                    *b &= oss == Tag::SymbolSize(s.len() as _);
                    ...
                    let Tag::SymbolSize(ss) = oss else {
                        unreachable!()
                    };
                    *b &= slice_from_raw_parts(
                        other.0.base.ptr.add(other.0.offset as usize + o + 1),
                        ss as usize,
                    )
                    .as_ref()
                    .unwrap()
                ...
                |b: &mut bool, o, a| {
                    *b &= *other.0.base.ptr.add(other.0.offset as usize + o)
                        == item_byte(Tag::Arity(a));
    ...
    while let Some((lhs, rhs)) = stack.pop() {
        ...
        if decomposable(&lhs, &rhs) {
            ...
            if let Some(&v) = st.memo.get(&key) {
                if PRINT_DEBUG {
                    println!("did find re-use ({}, {}) = {}", lhs.show(), rhs.show(), v);
                }
                oz.write_var_ref(v);
                oz.loc += 1;
            } else {
...
mod tests {
    ...
    fn test_unify() {
        ...
        {
            let mut tv = parse!(
                "[2] [2] flip [3] = $ [3] T _1 [4] a _1 $ $ [2] axiom [3] = [3] T _1 [4] a _1 _2 _3 _1"
            );
        ...
        {
            let mut srcv = parse!(
                r"[2] axiom [3] = [3] T $ [3] * $ _2 [3] T _1 [3] R [4] a _1 $ $ [3] * _2 _2"
            );
            let src = Expr {
                ptr: srcv.as_mut_ptr(),
            };
            ...
            let templ = Expr {
                ptr: templv.as_mut_ptr(),
            };

            let mut rv = parse!(
                r"[2] flip [3] = [3] T $ [3] R [4] a _1 $ $ [3] * $ _4 [3] T _1 [3] * _4 _4"
            );
            let r = Expr {
                ptr: rv.as_mut_ptr(),
            };
        ...
        // println!("================");
        {
            ...
            let mut xv = parse!(
                "[3] [3] [3] [3] [3] [3] a * $ * $ * $ * $ * $ = [3] _5 * [3] _4 * [3] _3 * [3] _2 * [3] _1 * a"
            );
            //                   ($res (axiom (= (T $x (a $x $y $z)) $x)))
            //                    result   input-data
            let x = Expr {
        ...
        {
            let mut xv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] * [3] / 1 $ [3] * _1 $ $ _1 [3] \\ _1 1 [3] * _2 [3] * [3] * _3 _1 [3] \\ _1 1"
            );
        ...
        {
            let mut xv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] / 1 $ $ $ _1 [3] \\ _1 [3] * _2 [3] * _3 _1"
            );
            //                   (axiom (= (* (* (* (* (/ 1 $) (* _1 $)) $) _1) (\ _1 1)) (* _2 (* (* _3 _1) (\ _1 1)))))
            ...
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] / 1 $ $ _1 [3] K $ $ [3] \\ _1 [3] * _2 [3] * _1 [3] K _3 _4"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        {
            let mut xv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] K $ $ [3] / 1 $ $ _3 [3] \\ _3 [3] * [3] K _1 [3] T _2 _3 [3] * _4 _3"
            );
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] K $ $ [3] / 1 $ $ _3 [3] \\ _3 [3] * [3] K _1 [3] T _2 [3] / 1 _3 [3] * _4 _3"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
            ...
            // occurs
            assert!(matches!(
                x.unify(y, &mut ExprZipper::new(to)),
                Err(UnificationFailure::Occurs(_, _))
            ));
        }
        {
            ...
            assert!(matches!(
                x.unify(y, &mut ExprZipper::new(to)),
                Err(UnificationFailure::Occurs((1, 1), _))
            ));
        }
        {
            let mut tv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * $ _1 _1 [4] L $ [4] L _1 _1 _1 [3] * _1 _1 [3] * [3] * _1 _1 [3] * [4] L [3] T _1 [3] * _1 _1 _1 _1 _2"
            );
            let t = Expr {
                ptr: tv.as_mut_ptr(),
            };
            ...
            let mut xv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * $ $ _1 [4] L $ [4] L _1 _1 _2 [3] * _2 _1 [3] * [3] * _2 _1 [3] * [4] L [3] T _1 [3] * _1 _2 _1 _2 _3"
            );
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * $ $ _1 [4] L $ [4] L _1 _1 _2 [3] * _2 _1 [3] * [3] * _2 _1 [3] * [4] L [3] T _1 [3] * _2 _1 _1 _2 _3"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        }
        {
            ...
            //
            let mut xv =
                parse!("[2] axiom [3] = [3] * [3] * [3] * $ $ _1 _2 [3] * _2 [3] * _1 [3] * _2 _1");
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * $ _1 $ [3] * [3] * _1 _1 _2 [3] * [3] * _1 [3] * _1 _2 [3] * _1 [3] * _1 _2"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        {
            let mut xv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * $ $ $ [3] K _2 _1 [3] T [3] * [3] * _2 _1 _3 [3] K _2 _1"
            );
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * $ $ $ [3] K _1 [4] L _2 [3] * _2 _1 _3 [3] T [3] * [3] * _2 _1 _3 [3] K _1 _2"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        }
        {
            ...
            let mut xv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] K $ $ $ $ [4] a _2 _1 [3] K _4 _3 [3] * [3] * [3] K _1 [4] L _2 _4 _3 _4 [3] T _3 _4"
            );
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * [3] * [3] K $ $ $ $ [4] a _1 _4 [3] K _3 [3] T _2 _4 [3] * [3] * [3] K _1 [4] L _2 _4 _3 _4 [3] T _3 _4"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        {
            let mut xv =
                parse!("[2] axiom [3] = [3] * [3] * $ [3] * $ _2 _2 [3] * _1 [3] * [3] * _2 _2 _2");
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * $ [3] * $ [3] \\ $ 1 [3] \\ [3] \\ _3 1 1 [3] * [3] / 1 _3 [3] * [3] * _3 _1 _2"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        {
            let mut xv =
                parse!("[2] axiom [3] = [3] * [3] * $ [3] T $ $ _3 [3] * [3] * _1 _2 [3] T _3 _2");
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                "[2] axiom [3] = [3] * [3] * $ [3] T $ $ [3] T [3] T _2 _3 $ [3] * [3] * _1 _2 [3] T _2 [3] T _4 _3"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
        ...
        {
            let mut xv = parse!(
                r"[2] axiom [3] = [3] * [3] K $ [3] * [3] \ $ [3] \ _2 $ [3] / $ 1 [3] T [3] T _1 [3] \ _2 [3] \ _2 [3] \ [3] \ _4 [3] \ _2 [3] \ _2 _3 [3] \ _2 [3] \ _2 _3 [3] * _3 [3] \ _2 [3] \ _2 [3] \ [3] \ _4 [3] \ _2 [3] \ _2 _3 [3] \ _2 [3] \ _2 _3 [3] T _1 [3] \ _2 [3] \ _2 [3] \ [3] \ _4 [3] \ _2 [3] \ _2 _3 [3] \ _2 [3] \ _2 _3"
            );
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
        ...
        {
            let mut xv = parse!(
                r"[4] exec PC0 [4] , [2] petri [4] ? $ $ $ [2] petri [3] ! _1 _2 [4] exec PC0 $ $ [3] , [2] petri _3 [4] exec PC0 _4 _5"
            );
            let x = Expr {
                ptr: xv.as_mut_ptr(),
            };
            let mut yv = parse!(
                r"[4] exec PC0 [4] , [2] petri [4] ? [2] add $ [2] [2] S $ $ [4] ? [2] add $ [2] _2 _3 [3] ! _1 [2] S _4 [2] petri [3] ! [2] add result [2] [2] S Z [2] S Z [4] exec PC0 [4] , [2] petri [4] ? $ $ $ [2] petri [3] ! _5 _6 [4] exec PC0 $ $ [3] , [2] petri _7 [4] exec PC0 _8 _9 [3] , $ $"
            );
            let y = Expr {
                ptr: yv.as_mut_ptr(),
            };
    ...
    fn rel_eq() {
        {
            ...
            let lhs = RelExprEnv(ExprEnv {
                n: 0,
                v: 1,
                offset: 1 + 2 + 1,
                base: e,
            });
            let rhs = RelExprEnv(ExprEnv {
                n: 0,
                v: 2,
                offset: 1 + 2 + 1 + 1 + 1 + 1,
                base: e,
            ...
            assert_eq!(
                {
                    let mut hx = XxHasher::with_seed(0);
                    lhs.hash(&mut hx);
                    hx.finish()
                },
                {
