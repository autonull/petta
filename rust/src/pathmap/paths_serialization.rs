    ...
    W: std::io::Write,
{
    ...
    #[allow(invalid_value)]
    //Squish the warning about a Null function ptr, because zlib uses a default allocator if the the ptr is NULL
    ...
    'reading: loop {
        ...
        'decompressing: loop {
            ...
            if ret == Z_STREAM_ERROR {
                return Err(std::io::Error::new(std::io::ErrorKind::Other, "Z_STREAM_ERROR"));
            }
            if strm.avail_out as usize == OUT {
                if ret == Z_STREAM_END { break 'reading } else { continue 'reading }
            }
            ...
            'descending: loop {
                if finished_path {
                    ...
                    if lbuf_offset == 4 {
                        ...
                        if strm.avail_in == 0 { continue 'reading } else { continue 'decompressing }
                    }
                }
                ...
                if pos + l as usize <= end {
                    ...
                    pos += l as usize;
                    finished_path = true;
                    if pos == end { continue 'decompressing } else { continue 'descending }
                } else {
...
mod test {
    use super::zipper::{ZipperIteration, ZipperMoving, ZipperValues};
    use super::*;
                ...
                match deserialize_paths(restored_btm.write_zipper(), v.as_slice(), ()) {
                    Ok(DeserializationStats { bytes_in: c, bytes_out: bw, path_count: pw }) => {
                        ...
                        while lrz.to_next_val() {
                            assert!(
                                btm.contains(lrz.path()),
                                "{}",
                                std::str::from_utf8(lrz.path()).unwrap()
                            );
                        }
