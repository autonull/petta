...
pub trait ZipperMoving: Zipper {
    ...
    fn descend_last_byte(&mut self) -> bool {
        ...
        if cc == 0 {
            ...
            self.descend_indexed_byte(cc - 1)
        }
    }
...
pub(crate) mod read_zipper_core {
        ...
        for ReadZipperCore<'trie, '_, V, A>
    {
        ...
        fn descend_indexed_byte(&mut self, child_idx: usize) -> bool {
            ...
            match self
                .focus_node
                .nth_child_from_key(self.node_key(), child_idx)
            {
        ...
        fn descend_to_existing<K: AsRef<[u8]>>(&mut self, k: K) -> usize {
            ...
            if key_start < self.prefix_buf.len()
                && !self
                    .focus_node
                    .node_contains_partial_key(&self.prefix_buf[key_start..])
            {
                return 0;
            }
            ...
            let overlap = if !node_key.is_empty() {
        ...
        for ReadZipperCore<'trie, '_, V, A>
    {
        ...
        fn reserve_buffers(&mut self, path_len: usize, stack_depth: usize) {
            ...
            if self.ancestors.capacity() < stack_depth {
                self.ancestors
                    .reserve(stack_depth.saturating_sub(self.ancestors.len()));
            }
        }
    }
                ...
                if let Some((parent, _iter_tok, _prefix_offset)) = self.ancestors.last() {
                    ...
                    if self.root_val.is_some() {
                        ...
                        witness
                            ...
                            .unwrap()
                            .as_tagged()
                            .node_get_val(self.root_node_key())
                    }
                }
            }
        }
        ...
        ReadZipperCore<'a, 'path, V, A>
    {
        ...
        pub(crate) fn new_with_node_and_path_internal_in(
            ...
            if path.len() > root_key_start
                && let Some((consumed_byte_cnt, next_node)) =
        ...
        fn descend_to_internal(&mut self, k: &[u8]) -> (&Self, &[u8]) {
            ...
            while let Some((consumed_byte_cnt, next_node)) = self.focus_node.node_get_child(key) {
                ...
                self.ancestors
                    .push((*self.focus_node.clone(), NODE_ITER_INVALID, key_start));
                *self.focus_node = next_node;
        ...
        fn ascend_within_node(&mut self) {
            ...
            let new_len = self
                .origin_path
                .len()
                .max(self.node_key_start() + branch_key.len());
            self.prefix_buf.truncate(new_len);
        }
...
pub(crate) mod zipper_moving_tests {
    ...
    pub fn zipper_moving_basic_test<Z: ZipperMoving>(mut zipper: Z) {
        ...
        assert_eq!(
            zipper.child_mask().iter().collect::<Vec<_>>(),
            vec![b'\'', b'a', b'u']
        ); // all three options we visited
    ...
    pub fn zipper_child_mask_test2<Z: ZipperMoving>(mut zipper: Z) {
        ...
        assert_eq!(
            ...
            [
                0,
                1 << (b'a' - 64) | 1 << (b'b' - 64) | 1 << (b'c' - 64) | 1 << (b'r' - 64),
                0,
                0
            ]
        );
    ...
    pub const ZIPPER_BYTES_ITER_TEST3_KEYS: &[&[u8]] = &[
        &[3, 193, 4, 193, 5, 2, 193, 6, 193, 7],
        &[3, 193, 4, 193, 5, 2, 193, 6, 255],
    ];
...
pub(crate) mod zipper_iteration_tests {
    ...
    pub fn k_path_test5<'a, Z: ZipperIteration>(mut zipper: Z) {
        ...
        assert_eq!(
            zipper.path(),
            &[3, 193, 4, 194, 1, 43, 3, 193, 8, 194, 1, 45, 194]
        );
    }
...
mod tests {
    ...
    fn full_path_test() {
        ...
        assert_eq!(
            format!("roma{}", std::str::from_utf8(zipper.path()).unwrap()),
            "roma"
        );
        ...
        assert_eq!(
            std::str::from_utf8(zipper.origin_path()).unwrap(),
            "romanus"
        );
        zipper.to_next_val();
    ...
    fn read_zipper_is_shared_test1() {
        ...
        assert_eq!(
            map.val_count(),
            l0_keys.len() * l1_keys.len() * l2_keys.len()
        );
