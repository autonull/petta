...
fn list_node_take_child_to_drop<V: Clone + Send + Sync, A: Allocator>(
    ...
    match (child0, child1) {
        (true, false) => {
            ...
            if !next_node.is_empty() { Some(next_node) } else { None }
        }
        (false, true) => {
            if node.is_used::<0>() {
...
impl<V: Clone + Send + Sync, A: Allocator> core::fmt::Debug for LineListNode<V, A> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let dangling_0 = if self.is_used_child_0() {
            if unsafe { self.child_in_slot::<0>() }.is_empty() { "true" } else { "false" }
        } else {
            "n/a"
        };
        ...
        let dangling_1 = if self.is_used_child_1() {
            if unsafe { self.child_in_slot::<1>() }.is_empty() { "true" } else { "false" }
        } else {
            "n/a"
        };
...
impl<V: Clone + Send + Sync, A: Allocator> LineListNode<V, A> {
    ...
    fn header0(is_child_ptr: bool, key_len: usize) -> u16 {
        let mask = if is_child_ptr { 0xa000 | (key_len << 6) } else { 0x8000 | (key_len << 6) };
        mask as u16
    }
    /// Constructs a header for slot1
    #[inline]
    fn header1(is_child_ptr: bool, key_len: usize) -> u16 {
        let mask = if is_child_ptr { 0x5000 | key_len } else { 0x4000 | key_len };
        mask as u16
    }
    ...
    pub(crate) fn get_both_keys(&self) -> (&[u8], &[u8]) {
        let key0 = if self.is_used::<0>() { unsafe { self.key_unchecked::<0>() } } else { &[] };
        let key1 = if self.is_used::<1>() { unsafe { self.key_unchecked::<1>() } } else { &[] };
        (key0, key1)
    }
            ...
            0 => match payload {
                ValOrChild::Child(child) => unsafe {
                    self.set_payload_0(
                        key,
                        true,
                        ValOrChildUnion { child: ManuallyDrop::new(child) },
                    )
                },
                ValOrChild::Val(val) => unsafe {
                    self.set_payload_0(
                        key,
                        false,
                        ValOrChildUnion { val: ManuallyDrop::new(LocalOrHeap::new(val)) },
                    )
                },
            },
            1 => match payload {
        ...
        let set_payload_recursive =
            |mut child: TaggedNodeRefMut<'_, V, A>, node_key, payload: ValOrChildUnion<V, A>| {
                if IS_CHILD {
                    ...
                    child.node_set_branch(node_key, onward_link).map(|_| (None, true))
                } else {
        ...
        ByteNode<Cf, A>: TrieNodeDowncast<V, A>,
    {
        ...
        if self.is_used::<0>() {
            ...
            if key_0.len() > 1 {
                ...
                replacement_node
                    .set_child(key_0[0], TrieNodeODRc::new_in(child_node, self.alloc.clone()));
            } else {
                if self.is_child_ptr::<0>() {
        ...
        if self.is_used::<1>() {
            ...
            if key_1.len() > 1 {
                ...
                replacement_node
                    .set_child(key_1[0], TrieNodeODRc::new_in(child_node, self.alloc.clone()));
            } else {
                if self.is_child_ptr::<1>() {
...
fn remaining_key(src_key: &[u8], key_len: usize) -> &[u8] {
    if src_key.len() > key_len { &src_key[key_len..] } else { &[] }
}
...
fn merge_list_nodes<V: Clone + Send + Sync + Lattice, A: Allocator>(
    ...
    if !used[0] {
        if let Some(payload) = a.clone_payload::<0>() {
    ...
    if !used[1] {
        if let Some(payload) = a.clone_payload::<1>() {
    ...
    if !used[2] {
        if let Some(payload) = b.clone_payload::<0>() {
    ...
    if !used[3] {
        if let Some(payload) = b.clone_payload::<1>() {
...
fn follow_path<'a, 'k, V: Clone + Send + Sync, A: Allocator + 'a>(
    ...
    if node.node_contains_partial_key(key) { Some((key, node)) } else { None }
}
...
fn follow_path_to_value<'a, 'k, V: Clone + Send + Sync, A: Allocator + 'a>(
    ...
    if node.node_contains_partial_key(key) { (false, Some((key, node))) } else { (false, None) }
}
...
impl<V: Clone + Send + Sync, A: Allocator> TrieNode<V, A> for LineListNode<V, A> {
    ...
    fn count_branches(&self, key: &[u8]) -> usize {
        ...
        let c0 =
            if key0.len() > key_len && starts_with(key0, key) { Some(key0[key_len]) } else { None };
        let c1 =
            if key1.len() > key_len && starts_with(key1, key) { Some(key1[key_len]) } else { None };
        match (c0, c1) {
            (None, None) => 0,
            (Some(_), None) => 1,
            (None, Some(_)) => 1,
...
mod tests {
    ...
    fn test_line_list_node() {
        ...
        *new_node.node_get_val_mut("hello".as_bytes()).unwrap() = 123;
        ...
        assert_eq!(
            new_node.node_set_val("hello".as_bytes(), [0, 1, 2, 3]).map_err(|_| 0),
            Ok((None, false))
        );
        ...
        *new_node.node_get_val_mut("hello".as_bytes()).unwrap() = [3, 2, 1, 0];
        ...
        assert_eq!(new_node.node_set_val("hello".as_bytes(), 42).map_err(|_| 0), Ok((None, false)));
        assert_eq!(
            new_node.node_set_val("goodbye".as_bytes(), 24).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(new_node.node_get_val("hello".as_bytes()), Some(&42));
    ...
    fn test_line_list_node_key_and_value_collision() {
        ...
        assert_eq!(
            child_node.node_set_val("hello".as_bytes(), 24).map_err(|_| 0),
            Ok((None, false))
        );
        ...
        unsafe {
            new_node.set_child_1("a".as_bytes(), TrieNodeODRc::new_in(child_node, global_alloc()));
        }
    ...
    fn test_line_list_node_shared_prefixes_empty() {
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val("my name is".as_bytes(), 42).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_set_val("my name is billy".as_bytes(), 24).map_err(|_| 0),
            Ok((None, true))
        );
    ...
    fn test_line_list_drop_head_releases_slot_1_availability() {
        ...
        let mut shortened =
            new_node.drop_head_dyn(drop_bytes).unwrap().as_tagged().as_list().unwrap().clone();
    ...
    fn test_line_list_node_shared_prefixes_slot_0() {
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val("I'm billy".as_bytes(), 42).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_set_val("slot1".as_bytes(), 123).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_set_val("I'm johnny".as_bytes(), 24).map_err(|_| 0),
            Ok((None, true))
        );
    ...
    fn test_line_list_node_shared_prefixes_slot_1() {
        let mut new_node = LineListNode::<usize, GlobalAlloc>::new_in(global_alloc());
        assert_eq!(
            new_node.node_set_val("slot0".as_bytes(), 123).map_err(|_| 0),
            Ok((None, false))
        );
        assert_eq!(
            new_node.node_set_val("I'm billy".as_bytes(), 42).map_err(|_| 0),
    ...
    fn test_line_list_node_replacement() {
        ...
        let (bytes_used, child_node) =
            replacement_node.as_tagged().node_get_child("apple".as_bytes()).unwrap();
        let child_node = child_node.as_tagged();
        assert_eq!(bytes_used, 1);
        assert_eq!(child_node.node_get_val("pple".as_bytes()), Some(&1));
        let (bytes_used, child_node) =
            replacement_node.as_tagged().node_get_child("banana".as_bytes()).unwrap();
        let child_node = child_node.as_tagged();
        assert_eq!(bytes_used, 1);
        assert_eq!(child_node.node_get_val("anana".as_bytes()), Some(&2));
        let (bytes_used, child_node) =
            replacement_node.as_tagged().node_get_child("carrot".as_bytes()).unwrap();
        let child_node = child_node.as_tagged();
        assert_eq!(bytes_used, 1);
    ...
    fn test_line_list_siblings_and_children() {
        ...
        assert_eq!(
            new_node.node_set_val("albatross".as_bytes(), 42).map_err(|_| 0),
            Ok((None, true))
        );
        debug_assert!(validate_node(&new_node));
        assert_eq!(new_node.node_set_val("a".as_bytes(), 42).map_err(|_| 0), Ok((None, false)));
