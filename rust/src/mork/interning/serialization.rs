use super::{
    MAX_WRITER_THREADS, PEARSON_BOUND, SYM_LEN, SYMBOL_THREAD_PERMIT_BYTE_POS, SharedMapping,
    SharedMappingHandle, Slab, Symbol, ThinBytes, bounded_pearson_hash,
};
use crate::pathmap::{PathMap, morphisms::Catamorphism};
use std::{
    io::{Read, Write},
    path::Path,
};
...
macro_rules! file_sizes_meta_filename {
    () => {
        "FileSizes.meta"
    };
}
...
impl SharedMapping {
    ...
    pub fn serialize(&self, out_path: impl AsRef<Path>) -> Result<(), std::io::Error> {
        ...
        for (n, lock) in locks.iter().enumerate() {
            ...
            buffer.start_file::<String, _>(
                format!("SharedMapping_0x{:0>2X}.binary_data", n as u8),
            ...
            reader.into_cata_jumping_side_effect::<Result<(), std::io::Error>, _>(
                |_, _, _, v, o| {
                    if let Some(ptr) = v {
        ...
        buffer.start_file::<&str, _>(
            FILE_SIZES_META_FILENAME,
    ...
    pub fn deserialize(in_path: impl AsRef<Path>) -> Result<SharedMappingHandle, std::io::Error> {
        ...
        let mut zip_archive = ZipArchive::new(file)
            .map_err(|_| std::io::Error::other("failed to read zip archive"))?;
        ...
        zip_archive
            .by_name(FILE_SIZES_META_FILENAME)?
            .read_to_end(&mut meta)?;
        let mut meta_slice = &meta[..];
        if meta_slice.len() as u32 % (2 * u64::BITS / u8::BITS) != 0 {
            return Err(std::io::Error::other("Malformed metadata file"));
        }

        loop {
            let Some((head, tail)) =
                meta_slice.split_at_checked((2 * u64::BITS / u8::BITS) as usize)
            else {
                break;
            };
            let (idx, size) = head.split_at((u64::BITS / u8::BITS) as usize);
            unsafe {
                file_sizes[u64::from_be_bytes(*(idx.as_ptr() as *const [u8; 8])) as usize] =
                    u64::from_be_bytes(*(size.as_ptr() as *const [u8; 8])) as usize
            }
        ...
        for file_name in files {
            if file_name.as_str() == FILE_SIZES_META_FILENAME {
                continue;
            }
            ...
            let match_error = || {
                Err(std::io::Error::other(
                    "Malformed filename, expected `SharedMapping_0x[0-9A-F]{2}\\.binary_data`",
                ))
            };
            let [top @ HEX!(), bot @ HEX!(), rest @ ..] = &file_name_bytes[LEADING.len()..] else {
                return match_error();
            };
            if &file_name_bytes[..LEADING.len()] != LEADING || rest != EXTENSION {
                return match_error();
            }
            ...
            let mut zip_file = zip_archive
                .by_name(&file_name)
                .map_err(|_| std::io::Error::other("File failed to be extracted from zip"))?;
            let slab_size = file_sizes[index];

            unsafe {
                let slab_ptr = Slab::allocate(slab_size as u64);
                (*mapping_ptr).permissions[index]
                    .0
                    .symbol_table_start
                    .store(slab_ptr, core::sync::atomic::Ordering::Relaxed);
                (*mapping_ptr).permissions[index]
                    .0
                    .symbol_table_last
                    .store(slab_ptr, core::sync::atomic::Ordering::Relaxed);
                ...
                let slab_slice = &mut *core::ptr::slice_from_raw_parts_mut(
                    (*slab_ptr).slab_data,
                    (*slab_ptr).slab_len,
                );
                ...
                let mut start = 0;
                while let diff @ 1..=usize::MAX =
                    zip_file.read(&mut slab_slice[start..slab_size])?
                {
                    start += diff;
                }
                ...
                while let Some((sym_bytes, to_parse_0)) =
                    to_parse.split_at_checked(SYM_LEN - SYMBOL_THREAD_PERMIT_BYTE_POS)
                {
                    let [s0, s1, s2, s3, s4, s5] = (sym_bytes.as_ptr()
                        as *const [u8; SYM_LEN - SYMBOL_THREAD_PERMIT_BYTE_POS])
                        .read();
                    ...
                    let (length, to_parse_2) = if (leading_byte as i8).is_negative() {
                        let Some((_, to_parse_1)) = to_parse_0.split_at_checked(1) else {
                            return Err(std::io::Error::other(concat!(
                                "Malformed data, expected length byte, file : ",
                                file!(),
                                ", line : ",
                                line!()
                            )));
                        ...
                        let Some((len_bytes, to_parse_1)) = to_parse_0.split_at_checked(8) else {
                            return Err(std::io::Error::other(concat!(
                                "Malformed data, expected 8 length bytes, file : ",
                                file!(),
                                ", line : ",
                                line!()
                            )));
                        ...
                        (
                            u64::from_be_bytes((len_bytes.as_ptr() as *const [u8; 8]).read())
                                as usize,
                            to_parse_1,
                        )
                    };
                    ...
                    to_symbol[bounded_pearson_hash::<{ PEARSON_BOUND }>(&to_parse[0..length])
                        as usize
                        % MAX_WRITER_THREADS]
                        .insert(&to_parse_2[0..length], sym);
                    to_bytes[index].insert(&sym, ThinBytes(to_parse_0.as_ptr()));

                    let Some((_, [to_parse_3 @ ..])) = to_parse_2.split_at_checked(length) else {
                        return Err(std::io::Error::other(concat!(
                            "Malformed data, unexpected end', file : ",
                            file!(),
                            ", line : ",
                            line!()
                        )));
                ...
                (*mapping_ptr).permissions[index]
                    .0
                    .next_symbol
                    .store(max_symbol + 1, core::sync::atomic::Ordering::Relaxed);
            }
        }
...
mod test {
    ...
    fn serialize_long() {
        ...
        const ALPHA_NUM: [[u8; ALPHA_NUM_LEN]; ARR_LEN] = [*b"abcdefghijklmnopqrstuvwxyz\
      ...
      "; ARR_LEN];
        ...
        load.to_bytes[0]
            ...
            .unwrap()
            .read_zipper()
            .into_cata_side_effect(|_mask, _accs, val: Option<&ThinBytes>, _path| {
                match val {
                    Some(id) => core::assert!(
                        unsafe { &(*id.as_raw_slice())[..] }
                            .iter()
    ...
    fn trivial_serialize() {
        ...
        for each in 0..MAX_WRITER_THREADS {
            ...
            handles.push(std::thread::spawn(move || {
                let handle_ = handle;
                let Ok(write_permit) = handle_.try_aquire_permission() else {
                    return;
                };
    ...
    unsafe fn as_btree(
    ...
    ) -> (BTreeMap<String, [u8; 8]>, BTreeMap<[u8; 8], String>) {
        ...
        for each in 0..MAX_WRITER_THREADS {
            ...
            for (value, path) in shared_mapping.to_bytes[each].0.read().unwrap().iter() {
                ...
                out2.insert(
                    unsafe { *(value.as_ptr() as *const [u8; SYM_LEN]) },
                    unsafe { core::mem::transmute((&*path.as_raw_slice()).to_owned()) },
                );
            }
        }
