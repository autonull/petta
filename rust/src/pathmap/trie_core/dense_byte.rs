        ...
        self.for_each_item(|self_node, key_byte, cf_idx| {
            if other.node_contains_partial_key(&[key_byte]) {
                ...
                if let Some(self_val) = cf.val()
                    && let Some(other_val) = other.node_get_val(&[key_byte])
                {
    ...
    ByteNode<Cf, A>: TrieNodeDowncast<V, A>,
{
    ...
    fn node_get_child(&self, key: &[u8]) -> Option<(usize, &TrieNodeODRc<V, A>)> {
        self.get(key[0])
            .and_then(|cf| cf.rec().map(|child_node| (1, child_node)))
    }
    fn node_get_child_mut(&mut self, key: &[u8]) -> Option<(usize, &mut TrieNodeODRc<V, A>)> {
        debug_assert!(!key.is_empty());
        self.get_child_mut(key[0])
            .map(|child_node_ptr| (1, child_node_ptr))
    }
        ...
        #[cfg(not(feature = "all_dense_nodes"))]
        {
            ...
            if key.len() > 1 {
                #[cfg(not(feature = "bridge_nodes"))]
                {
                    let mut child = super::line_list::LineListNode::new_in(self.alloc.clone());
                    child
                        .node_set_branch(&key[1..], new_node)
                        .unwrap_or_else(|_| panic!());
                    self.set_child(key[0], TrieNodeODRc::new_in(child, self.alloc.clone()));
                }
                #[cfg(feature = "bridge_nodes")]
    ...
    fn node_val_count(&self, cache: &mut HashMap<u64, usize>) -> usize {
        ...
        self.values.iter().rfold(0, |t, cf| {
            t + cf.has_val() as usize
                + cf.rec()
                    .map(|r| val_count_below_node(r, cache))
                    .unwrap_or(0)
        })
    }
    fn node_goat_val_count(&self) -> usize {
        self.values
            .iter()
            .rfold(0, |t, cf| t + cf.has_val() as usize)
    }
    ...
    fn node_first_val_depth_along_key(&self, key: &[u8]) -> Option<usize> {
        debug_assert!(!key.is_empty());
        self.get(key[0])
            .and_then(|cf| if cf.has_val() { Some(0) } else { None })
    }
    fn nth_child_from_key(
        ...
        if FORWARD {
            (
                self.mask.indexed_bit::<FORWARD>(n),
                self.values[n].rec().map(|cf| cf.as_tagged()),
            )
        } else {
            let idx = self.values.len() - n - 1;
            (
                self.mask.indexed_bit::<FORWARD>(n),
                self.values[idx].rec().map(|cf| cf.as_tagged()),
            )
        }
    }
    ...
    fn first_child_from_key(&self, key: &[u8]) -> (Option<&[u8]>, Option<TaggedNodeRef<'_, V, A>>) {
        ...
        (
            Some(&ALL_BYTES[prefix..=prefix]),
            cf.rec().map(|cf| cf.as_tagged()),
        )
    }
    ...
    fn get_sibling_of_child(
        ...
        (
            Some(sibling_key_char),
            cf.rec().map(|node| node.as_tagged()),
        )
    }
...
pub trait CoFree: Clone + Default + Send + Sync {
    ...
    fn both_mut(
        ...
        Option<&mut TrieNodeODRc<Self::V, Self::A>>,
        Option<&mut Self::V>,
    );
...
impl<V: Clone + Send + Sync, A: Allocator> Default for CellCoFree<V, A> {
    fn default() -> Self {
        Self(MaybeDangling::new(Box::pin(OrdinaryCoFree::new(
            None, None,
        ))))
    }
}
...
> HeteroDistributiveLattice<OtherCf> for Cf
{
        ...
        Self: Sized,
    {
        let self_rec = self
            .rec()
            .filter(|child| !child.as_tagged().node_is_empty());
