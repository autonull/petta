...
//!
//! Provides an ergonomic command-line interface with history and syntax highlighting
...
pub fn run_repl(config: &ReplConfig) {
    ...
    let mut rl = DefaultEditor::new().unwrap_or_else(|_| {
        eprintln!("{} Failed to initialize line editor", yellow("⚠"));
    ...
    print_welcome(config.backend);

    // Main REPL loop
    loop {
        match rl.readline(&green("metta> ").to_string()) {
            Ok(line) => {
                ...
                if handle_command(&line, &mut engine, config) {
                    break;
                }
            }
...
#[cfg(feature = "repl")]
fn print_welcome(backend: Backend) {
    println!("{} {}", cyan("⚡"), cyan("PeTTa REPL v0.5.0"));
    println!("Backend: {} ({})", cyan(&backend.to_string()), cyan("interactive"));
    println!("Type {} for available commands.\n", cyan(":help"));
}

#[cfg(feature = "repl")]
fn handle_command(line: &str, engine: &mut PeTTaEngine, config: &ReplConfig) -> bool {
    match line {
        ":quit" | ":q" => {
            println!("{}", green("Goodbye!"));
            true
        ...
        ":backend" | ":b" => {
            println!("Current backend: {}", yellow(&config.backend.to_string()));
            false
        }
        ":info" | ":i" => {
...
pub fn run_repl(_config: &ReplConfig) {
    eprintln!(
        "{} REPL support is disabled. Enable the 'repl' feature to use interactive mode.",
        red("✗")
    );
}
