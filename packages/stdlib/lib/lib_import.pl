...
% use-module is handled in TS / tau-prolog standard imports usually.
'use-module!'(Module, true) :- use_module(library(Module)).

%%% Git Import: %%%
% Delegate complex shell functionality to JS
'git-import!'(GitPath, true) :- js_call_predicate(['git_import', GitPath, '', './repos'], _).
'git-import!'(GitPath, BuildCmd, true) :- js_call_predicate(['git_import', GitPath, BuildCmd, './repos'], _).
'git-import!'(GitPath, BuildCmd, BaseDir, true) :- js_call_predicate(['git_import', GitPath, BuildCmd, BaseDir], _).
