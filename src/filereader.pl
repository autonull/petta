...
%Conditional printing helpers (only print when not in silent mode):
maybe_print_metta_sexpr(Term) :- silent(true), !.
maybe_print_metta_sexpr(Term) :- swrite(Term, STerm),
    format("\e[33m--> metta sexpr -->~n\e[36m~w~n\e[33m^^^^^^^^^^^^^^^^^^^~n\e[0m", [STerm]).

maybe_print_metta_runnable(_, Goals) :- silent(true), !, call_goals(Goals).
maybe_print_metta_runnable(FormStr, Goals) :-
...
maybe_print_metta_function(_, Ref) :- silent(true), !.
maybe_print_metta_function(FormStr, Ref) :-
