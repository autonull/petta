...
//!
//! Streamlined command-line interface with ergonomic defaults
...
fn main() {
    ...
    // Show demo if no files and not interactive
...
fn run_files(args: &Cli) {
    ...
    for path in &paths {
        if let Err(e) = process_file(&mut engine, path, &args.output_format) {
