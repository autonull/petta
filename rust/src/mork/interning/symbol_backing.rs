...
#[derive(
    Clone,
    Copy, /* Slab is just a header of an allocation, it does not "own" the next pointer */
)]
#[repr(C)]
...
impl Slab {
    pub(crate) unsafe fn allocate(bytes: u64) -> *mut Slab {
        ...
        let layout = std::alloc::Layout::array::<core::cell::UnsafeCell<u8>>(slab_size)
            .unwrap()
            .align_to(4096)
            .expect("Cannot be aligned");
    ...
    pub(crate) unsafe fn free(mut slab: *mut Self) {
        while !slab.is_null() {
            unsafe {
                ...
                std::alloc::dealloc(
                    cur as *mut u8,
                    std::alloc::Layout::array::<u8>(size)
                        .unwrap()
                        .align_to(4096)
                        .unwrap(),
                );
    ...
    pub(crate) fn as_raw_slice(self) -> *const [u8] {
        unsafe {
            ...
            let (ptr, len) = if ((1 << u8::BITS - 1) & tag) != 0 {
                ...
                // new version for serialization
                (
                    self.0.byte_add(U64_BYTES),
                    u64::from_be_bytes(*(self.0 as *const [u8; U64_BYTES])) as usize,
                )
