...
pub struct WritePermit<'a>(&'a SharedMappingHandle, PhantomData<*const ()>);
...
impl<'a> core::ops::Drop for WritePermit<'a> {
    fn drop(&mut self) {
        ...
        if permits == 0 {
            self.permissions[MAPPING_THREAD_INDEX.get().unwrap() as usize]
                .0
                .thread_id
        ...
        let sym = 'lock_scope_sym: {
            ...
            let thin_bytes_ptr = if let Some((thin_bytes, _)) = eager_and_recovery {
                ...
                if slab_ptr.is_null() {
                    ...
                    thread_permission
                        .symbol_table_start
                        .store(allocation, STORE_ORDER);
                    thread_permission
                        .symbol_table_last
                        .store(allocation, STORE_ORDER);
                    slab_ptr = allocation;
                }
                unsafe { Slab::register_bytes(slab_ptr, bytes) }
            };
            let new_sym = thread_permission
                .next_symbol
...
impl SharedMappingHandle {
    ...
    pub fn try_aquire_permission<'a>(&'a self) -> Result<WritePermit<'a>, ()> {
        ...
        for each in 0..MAX_WRITER_THREADS {
            ...
            if let Ok(_) = p.0.thread_id.compare_exchange(
                0,
                THREAD_ID.with(|x| *x) + 1,
...
impl core::ops::Drop for SharedMappingHandle {
    fn drop(&mut self) {
        ...
        let allocated = self.flags.load(atomic::Ordering::Acquire)
            & SharedMappingFlags::HeapAllocated as u64
            != 0;
        ...
        if allocated {
            unsafe {
                std::alloc::dealloc(
                    ...
                    std::alloc::Layout::new::<SharedMapping>(),
                )
            }
        }
    }
