...
use super::dense_byte::{CellByteNode, DenseByteNode, test_bit_in_mask};
use super::node::*;
...
impl<V> Debug for BridgeNode<V> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "BridgeNode (occupied={} is_child={} key={:?})",
            !self.is_empty(),
            self.is_child_ptr(),
            key
        )
...
impl<V: Clone + Send + Sync> BridgeNode<V> {
        ...
        if key.len() <= KEY_BYTES_CNT {
            ...
            unsafe {
                core::ptr::copy_nonoverlapping(
                    key.as_ptr(),
                    new_node.key_bytes.as_mut_ptr().cast(),
                    key.len(),
                );
            }
            new_node
    ...
    fn splice_new_payload<const IS_CHILD: bool>(
        &mut self,
        key: &[u8],
        mut new_payload: ValOrChildUnion<V>,
    ) -> Result<(Option<V>, bool), TrieNodeODRc<V>> {
        debug_assert_eq!(self.is_empty(), false);
        ...
        if overlap > 0 {
            ...
            if overlap > 0 {
                self.set_payload(
                    &key[..overlap],
                    true,
                    TrieNodeODRc::new(replacement_node).into(),
                );
                Ok((None, true))
            } else {
    ...
    fn merge_bridge_nodes(&self, other: &BridgeNode<V>) -> TrieNodeODRc<V>
    where
        V: Lattice,
    {
        debug_assert!(!self.is_empty());
        ...
        if overlap > 0 {
            ...
            if overlap == self_key.len()
                && overlap == other_key.len()
                && self_is_child == other_is_child
            {
                if self_is_child {
            ...
            if overlap > 0 {
                let new_node = Self::new(
                    &self_key[..overlap],
                    true,
                    TrieNodeODRc::new(split_node).into(),
                );
                return TrieNodeODRc::new(new_node);
            } else {
    ...
    fn convert_to_cell_node(&mut self, capacity: usize) -> TrieNodeODRc<V>
    where
        V: Clone,
    {
        debug_assert!(!self.is_empty());
...
impl<V: Clone + Send + Sync> TrieNode<V> for BridgeNode<V> {
    ...
    fn node_set_branch(
        &mut self,
        key: &[u8],
        new_node: TrieNodeODRc<V>,
    ) -> Result<bool, TrieNodeODRc<V>> {
        self.splice_new_payload::<true>(key, new_node.into())
    ...
    fn get_sibling_of_child(
        ...
        _key: &[u8],
        _next: bool,
    ) -> (Option<u8>, Option<&dyn TrieNode<V>>) {
    ...
    fn join_dyn(&self, other: &dyn TrieNode<V>) -> TrieNodeODRc<V>
    where
        V: Lattice,
    {
        debug_assert!(!self.is_empty());
        match other.as_tagged() {
            ...
            TaggedNodeRef::TinyRefNode(tiny_node) => tiny_node.join_dyn(self),
    ...
    fn join_into_dyn(&mut self, _other: TrieNodeODRc<V>) -> Result<(), TrieNodeODRc<V>>
    where
        V: Lattice,
    {
    ...
    fn drop_head_dyn(&mut self, _byte_cnt: usize) -> Option<TrieNodeODRc<V>>
    where
        V: Lattice,
    {
        unimplemented!()
    }
    fn meet_dyn(&self, _other: &dyn TrieNode<V>) -> Option<TrieNodeODRc<V>>
    where
        V: Lattice,
    {
        unimplemented!()
    }
    fn psubtract_dyn(&self, _other: &dyn TrieNode<V>) -> (bool, Option<TrieNodeODRc<V>>)
    where
...
fn test_bridge_node() {
    ...
    let new_node = BridgeNode::<usize>::new(
    ...
    let (consumed_bytes, child_node) = new_node
        .node_get_child(b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz")
        .unwrap();
    ...
    let check_val = new_node
        .node_get_val(b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcde")
        .unwrap();
    assert_eq!(*check_val, 42);
}
