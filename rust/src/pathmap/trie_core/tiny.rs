...
impl<'a, V: Clone + Send + Sync, A: Allocator> TinyRefNode<'a, V, A> {
    ...
    pub fn into_list_node(&self) -> Option<super::line_list::LineListNode<V, A>> {
        self.clone_payload().map(|payload| {
            ...
            unsafe {
                new_node.set_payload_owned::<0>(self.key(), payload);
            }
    ...
    fn header(is_child: bool, key_len: usize) -> u8 {
        ...
        if is_child { ((1 << 7) | (1 << 6) | key_len) as u8 } else { ((1 << 7) | key_len) as u8 }
    }
...
impl<'a, V: Clone + Send + Sync, A: Allocator> TrieNode<V, A> for TinyRefNode<'a, V, A> {
    ...
    fn next_items(
        &self,
        _token: u128,
    ) -> (u128, &'a [u8], Option<&TrieNodeODRc<V, A>>, Option<&V>) {
        unreachable!()
    }
    ...
    fn get_node_at_key(&self, key: &[u8]) -> AbstractNodeRef<'_, V, A> {
        ...
        if node_key.len() > key.len() && starts_with(node_key, key) {
            ...
            let ref_node =
                TinyRefNode::new_in(self.is_child_ptr(), new_key, self.payload, self.alloc.clone());
    ...
    fn drop_head_dyn(&mut self, _byte_cnt: usize) -> Option<TrieNodeODRc<V, A>>
    where
        V: Lattice,
    {
        unreachable!()
