...
pub(crate) const U64_BYTES: usize = u64::BITS as usize / 8;
...
///
pub(crate) type Symbol = [u8; SYM_LEN];
#[doc(hidden)]
...
pub(crate) const SYMBOL_THREAD_PERMIT_BYTE_POS: usize = 2;
...
impl SharedMapping {
    ...
    pub fn new() -> SharedMappingHandle {
        unsafe {
            let ptr = std::alloc::alloc(std::alloc::Layout::new::<MaybeUninit<SharedMapping>>())
                as *mut MaybeUninit<SharedMapping>;
    ...
    pub const unsafe fn init(
        uninit: *mut MaybeUninit<SharedMapping>,
        init_flags: u64,
    ) -> SharedMappingHandle {
        unsafe {
            ...
            while i <= MAX_WRITER_THREAD_INDEX {
                (&raw mut (*inner).permissions[i])
                    .write(AlignCache(ThreadPermission::init(i as u8)));
                (&raw mut (*inner).to_symbol[i])
                    .write(AlignCache(std::sync::RwLock::new(PathMap::new())));
                (&raw mut (*inner).to_bytes[i])
                    .write(AlignCache(std::sync::RwLock::new(PathMap::new())));
    ...
    pub unsafe fn keep_slabs_alive(&self) {
        self.flags.store(
            SharedMappingFlags::KeepSlabsAlive as u64,
...
impl core::ops::Drop for SharedMapping {
    fn drop(&mut self) {
        if self.flags.load(atomic::Ordering::Acquire) & SharedMappingFlags::KeepSlabsAlive as u64
            != 0
        {
...
impl ThreadPermission {
    const fn init(index: u8) -> ThreadPermission {
        core::debug_assert!(
            index < 0b_1000_0000,
            "The top bit of a symbol must be kept off."
        );
        let next_symbol_val = if index == 0 {
            1 /* We want to leave the 0 case clear, as that represents the De Bruijn variable introduction */
        } else {
            (index as u64)
                << (u64::BITS - u8::BITS * 3/* leave the top two bytes free for encoding in the pathmap the type/len, the third byte has the map index, the last 5 bytes leave the possibility for 2^40 symbols */)
        };
        ThreadPermission {
...
pub(crate) fn bounded_pearson_hash<const SELECTION: usize>(bytes: &[u8]) -> u8 {
